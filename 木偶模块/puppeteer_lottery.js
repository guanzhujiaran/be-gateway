		...
		if (
			...
			for (let retry = 0; retry < 5; retry++) {
				...
				try {
					...
					console.error(
						`${lottery_setting.CONFIG.COOKIENAME}\t浏览器启动失败\n${e}`
					);
					await sleep(10e3);
					continue;
				}
				...
				async function Daily_rewards() {
					let MyDailyFuncMap = {
						...
						VipGetBCoin: async () => {
							try {
								let vipStatus =
									global_var.user_nav?.data?.vipStatus;
								let vipType =
									global_var.user_nav?.data?.vipType;
								...
								console.error(
									`${global_var.user_info.uname}\t领取5b币失败！`,
									e
								);
							}
						},
					...
					if (lottery_setting.CONFIG.AUTO_DailyReward) {
						try {
							...
							console.error(
								`${global_var.user_info.uname}\t三连失败！`,
								e
							);
						}
