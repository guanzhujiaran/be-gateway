...
class DO_Lottery {
	 ...
	 * @param {number} broswer_mode - 浏览模式，true为只打开浏览器不抽奖
					...
					switch (opus_init.module_type) {
						case "MODULE_TYPE_AUTHOR": {
							polymer_detail_data.item.modules.module_author =
								opus_init.module_author;
							polymer_detail_data.item.modules.module_author.official_verify =
								opus_init.module_author.official;
							break;
						}
						case "MODULE_TYPE_CONTENT": {
							let text = [];
							for (let node of opus_init.module_content
								.paragraphs[0].text.nodes) {
								switch (node.type) {
					...
					async function CheckRisk() {
						...
						if (captcha || comment_dyn_response_code) {
							...
							console.warn(
								`${
									global_var.user_info.uname
								}\t动态${await this.global_var.page.url()} 评论失败，需要验证码，休眠4小时！\t${new Date().toLocaleTimeString()}`
							);
...
答：`;
		...
		/**
		 *
		 * @returns {boolean} true 代表账号信息获取成功
		 */
		async function account_init() {
			let cookieStr;
			try {
						...
						if (
							...
							for (let i = 0; i <= 5; i++) {
								...
								if (
									...
									// }
									let follow_pg =
										await global_var.browser.newPage();
									try {
