		...
		let utl = this.utl
			...
			: {
					...
					checkAuditTime: function (beginTime, endTime) {
						...
						var endMinue = endTime.substring(
							endIndex + 1,
							endTime.length
						);
					...
					part_shuffle: function (shuffle_num, shuffle_list) {
						...
						for (var i = 0; i < shuffle_num; i++) {
							var rdm = Math.floor(
								Math.random() * shuffle_list.length
							);
					...
					Get_Opus_Dynamic_Data: async function () {
						...
						polymer_detail_data.item.basic =
							opus_init_detail.detail.basic;
						...
						for (let m of opus_init_detail.detail.modules) {
							switch (m.module_type) {
								...
								case "MODULE_TYPE_CONTENT": {
									let text = [];
									for (let paragraph of m.module_content
										.paragraphs) {
										if (paragraph.para_type == 1) {
											for (let node of paragraph.text
												.nodes) {
												switch (node.type) {
						...
						polymer_detail_data.item.type =
							opus_init_detail.detail.type;
					...
					noRepeatArr: function (arr) {
						...
						try {
							...
							console.warn(
								e,
								`${global_var.user_info.uname}\tnoRepeat`
							);
						}
						return newArr;
					},
		...
		let my_operator = !this.my_operator
			? {
					basic_operator: {
						...
						check_reply: async function (
							...
							if (response_json) {
								try {
									...
									let reply_jump_resp =
										await MYAPI.BiliAPI.reply_jump(
											type,
											oid,
											rpid
										);
							...
							if (dynamic_id) {
								...
								if (!global_var.response.global_dynamic_data) {
									...
									dynamic_detail_res_data =
										dynamic_detail_res.data;
								}
								let comment_id_str =
									dynamic_detail_res_data.item.basic
										.comment_id_str;
								let comment_type =
									dynamic_detail_res_data.item.basic
										.comment_type;
						...
						comment_submit: async function (
							...
							try {
								if (
									Math.random() <
									lottery_setting.comment_thumb_chance
								) {
					...
					dynamic_content_operator: {
						//获取动态信息相关操作
						get_dynamic_content_and_top_msg: async function (
							dynamic_data
						) {
							//获取动态内容和up置顶的回复
							...
							try {
								...
								if (dynamic_type == "DYNAMIC_TYPE_AV") {
									...
									try {
										dynamic_content2 =
											dynamic_data.item.modules
												.module_dynamic.desc.additional;
									} catch {
										dynamic_content2 = "";
									}
									try {
					...
					dynamic_comment_operator: {
						...
						pre_msg_processing: function (
							...
							let topobj_6 =
								/.*@.{0,3}位.*|.*@.{0,3}名.*/gim.exec(
									non_topic_content
								);
							let topobj_5 =
								/.*@.{0,3}1位.*|.*@.{0,3}1名.*/gim.exec(
									non_topic_content
								);
							let topobj_4 =
								/.*@.{0,3}一位.*|.*@.{0,3}一名.*/gim.exec(
									non_topic_content
								);
							let topobj_3 =
							...
							let topobj6 = /.*艾特好友.*/gim.exec(
								non_topic_content
							);
							let topobj7 =
								/.*@.{0,4}名好友.*|.*@.{0,4}位好友.*/gim.exec(
									non_topic_content
								);
							...
							let topobj13 =
								/.*加话题#.{0,30}#((?!投稿).)*$/gim.exec(
									non_topic_content
								);
							let topobj14 =
								/.*带标签#.{0,30}#((?!投稿).)*$/gim.exec(
									non_topic_content
								);
							let topobj15 =
								/.*@三位好友.*|.*@三名好友.*/gim.exec(
						...
						manual_reply_judge: function (dynamic_content) {
							...
							let manual_re16 =
								/.*评论.{0,10}聊.{0,10}聊/gim.test(
									dynamic_content
								);
							let manual_re17 = /.*评.{0,10}接力/gim.test(
								dynamic_content
							);
							let manual_re18 = /.*聊.{0,10}聊/gim.test(
								dynamic_content
							);
							let manual_re19 =
							...
							let manual_re35 = /.*生日快乐/gim.test(
								dynamic_content
							);
							let manual_re36 = /.*一句话形容/gim.test(
								dynamic_content
							);
							let manual_re38 =
							...
							let manual_re48 = /.*唠.{0,5}唠/gim.test(
								dynamic_content
							);
							let manual_re49 = /.*分享一下/gim.test(
								dynamic_content
							);
							...
							let manual_re53 =
								/.*发布.{0,20}图.{0,5}动态/gim.test(
									dynamic_content
								);
							let manual_re54 = /.*视频.{0,20}评论/gim.test(
								dynamic_content
							);
							let manual_re55 = /.*复zhi|.*长按/gim.test(
								dynamic_content
							);
							let manual_re56 = /.*多少.{0,10}合适/gim.test(
								dynamic_content
							);
							let manual_re57 = /.*喜欢.{0,5}哪/gim.test(
								dynamic_content
							);
							let manual_re58 =
							...
							let manual_re61 = /.*看.{0,10}猜/gim.test(
								dynamic_content
							);
							let manual_re63 =
							...
							let manual_re73 =
								/.*知道.{0,15}什么.{0,15}？|.*知道.{0,15}什么.{0,15}\?|.*用什么|.*评.{0,10}收.{0,5}什么.{0.7}\?|.*评.{0,10}收.{0,5}什么.{0,7}？/gim.test(
									dynamic_content
								);
							let manual_re74 =
						...
						key_word_reply: function (dynamic_content) {
							...
							if (
								...
								return lottery_setting.key_word_comment
									.red_pocket;
							}
						...
						reply_comment_generator: async function (
							...
							let official_type =
								global_var.response.global_dynamic_data.item
									.modules.module_author.official_verify.type;
							if (!comment_msg) {
					...
					log_record: {
						construct_comment_record_data: async function (
							comment_msg
						) {
							let rep_dynamic_id = "";
							try {
					...
					prevent_filter_module: {
						...
						get_video_list: async function (__share_num) {
							...
							while (1) {
								if (
									share_video_list.length > __share_num * 5 ||
									bt
								) {
									break;
								}
						...
						share_video: async function (
							...
							if (
								...
								lottery_setting.prevent_module
									.share_video_switch &&
								lottery_setting.FLAG.share_flag
							) {
								let video_list = await this.get_video_list(
									share_num
								);
								let share_video_list = [];
						...
						create_word_dynamic_from_dynamic_main_page:
							async function (content_list, create_times) {
								let now = new Date();
								if (
									now.getHours() >= 0 &&
									now.getHours() <= 22
								...
								console.log(
									`${global_var.user_info.uname}\t分享彩虹屁`
								);
								let content;
								if (!create_times) {
									create_times = 1;
						...
						create_topic_dynamic_from_dynamic_main_page:
							async function (
								create_times,
								discuss_content,
								copy_discuss_flag
								...
								for (let i = 0; i < create_times; i++) {
									...
									relevant_topic__titles.splice(
										relevant_topic__titles.indexOf(
											extract_topic
										),
										1
									); //选好的话题就删掉
					...
					copy_reply_module: {
						...
						get_copy_reply: async (
							...
							for (let _ = 0; _ < 3; _++) {
								...
								all_replies_content =
									all_replies_content.concat(reps.ret_list);
							...
							if (all_replies_content.length >= 15) {
								ret_reply =
									utl.weight_rand(all_replies_content);
							}
							return ret_reply;
						},
						/**
						...
						get_reply_list: async (
							...
							if (
								...
								if (
									...
									comment_id_str =
										dynDetail_data.item.basic
											.comment_id_str;
									comment_type =
										dynDetail_data.item.basic.comment_type;
									reply_count =
										dynDetail_data.item.modules.module_stat
											.comment.count;
									try {
										up_mid =
											dynDetail_data.item.modules
												.module_author.mid;
									} catch (e) {
										console.error(
											e,
							...
							if (
								...
								up_mid =
									global_var.response.reply_main.data.upper
										.mid;
							}
							try {
							...
							for (
								...
								//     }
								// })
								for (let ignore_str of my_operator
									.copy_reply_module.ignore_replies) {
									if (
										replies_content[repindex] == ignore_str
									) {
										bf = true;
										break;
									}
						...
						string_semantic: async (input_str) => {
							try {
								let url =
									"http://127.0.0.1:23333/damo/semantic/";
