...
const {pptr_op, sleep} = require("./util/common_utl");
...
const file_map = ['cookie_file', 'log', '抽奖记录', "抽奖记录/官方抽奖记录", "抽奖记录/必抽的大奖记录", "抽奖记录/必抽的预约抽奖记录", 'JsonData']
file_map.map(el => {
    if (!fs.existsSync(__dirpath + el)) {
        //创建文件目录
        fs.mkdirSync(__dirpath + el);
    }
})

/**
 * Enum for usage values.
 * @readonly
 * @enum {number}
 */
const usage_map = {
    Lottery: 1, Reserve: 2, Follow: 3, DoUnFollow: 4, LiveAnchor: 5, LiveRedPacket: 6, BrowserMode: 7
}
...
class GlobalVar {
    ...
    TIME = {
        ...
        Reserve_Lottery_time: 30 * 3600 * 24, Last_Start_Ts: Math.ceil(Date.now() / 1e3),
    }
    /**
     *
    ...
    FLAG = {
        吃饭休息标志: false, 评论响应标志: false, opus动态标志: false, 抽奖中标志: false,
    }
    user_nav = undefined
    fengkong_flag = false //风控标志
    recorded_data = "" //抽奖反馈信息
    user_info = {
        uid: undefined, uname: undefined,
    }
    Pause = false //抽奖暂停标志
    Baidu_wenxin = {
        ...
        let my_operator = {
            basic_operator: {
                    ...
                    if (response_json) {
                        try {
                            if (response_json.code === 12051) {
                                console.warn(`${global_var.user_info.uname}\t检查评论失败！但重复评论，算作成功！`, JSON.stringify(response_json));
                                return true;
                            }
                            ...
                            console.warn(`${global_var.user_info.uname}\t检查评论失败！${e}`, JSON.stringify(response_json));
                ...
                get_opus_dynamic_repost_area_content: async (msg_box_node) => {
                    return await msg_box_node.$eval(`.bili-rich-textarea__inner`, async (el) => {
                        let ret_msg = "";
                ...
                dynamic_thumb: async (opus_dynamic = false) => {
                    ...
                    try {
                        if (typeof global_var.recorded_data == "string") {
                            if (global_var.recorded_data.includes("动态评论失败，评论被隐藏")) {
                                console.warn(`${global_var.user_info.uname}\t${global_var.current_page.url()}动态评论失败，评论被隐藏，不进行动态点赞！\t${new Date().toLocaleTimeString()}`);
                                return;
                            }
                        }
                        let pageurl = await global_var.current_page.url();
                        opus_dynamic = !!pageurl.includes("opus");

                        if (opus_dynamic || 1) {
                            for (let i = 0; i < 2; i++) {
                                await sleep(2e3);
                                await global_var.current_page.click(`.side-toolbar__action.like`);
                                await sleep(1e3);
                                if (await global_var.current_page
                                    .waitForSelector(`.side-toolbar__action.like.is-active`)
                                    .then((el) => true)
                                    .catch((e) => {
                                        console.error(`${global_var.user_info.uname}\t${pageurl}\t动态点赞图标获取失败`);
                                        return false;
                                    })) {
                                    ...
                                    console.error(`${global_var.user_info.uname}\t${global_var.current_page.url()}\t动态点赞失败`);
                                    await sleep(2e3);
                                }
                            }
                        }
                        // else {
                        // 	await sleep(2e3);
                        // 	await global_var.current_page_info.page.click(
                        // 		".bili-dyn-action.like"
                        // 	);
                        // 	await sleep(1e3);
                        // 	console.log(
                        // 		`${global_var.user_info.uname}\t${global_var.current_page_info.page.url()}\t动态点赞成功`
                        // 	);
                        // 	await sleep(
                        // 		utl.random_choice(
                        ...
                        console.warn(`${global_var.user_info.uname}\t${global_var.current_page.url()}\t动态点赞失败`, e);
                        await my_operator.log_record.my_throw(`${global_var.user_info.uname}\t${global_var.current_page.url()}\t动态点赞失败`);
                    }
                    ...
                    //     await my_operator.log_record.my_throw(`动态点赞失败，dynamic_thumb`, global_var.response.dynamic_thumb_response)
                 ...
                 * @returns
                 */
                dynamic_repost: async (opus_dynamic = false, repost_content = "") => {
                    if (typeof global_var.recorded_data == "string") {
                        if (global_var.recorded_data.includes("动态评论失败，评论被隐藏")) {
                            console.error(`${global_var.user_info.uname}\t${global_var.current_page.url()} 动态评论失败，评论被隐藏，不进行动态转发！\t${new Date().toLocaleTimeString()}`);
                            return;
                        }
                    }
                    //点击转发
                    await pptr_op.check_page_is_front(global_var.current_page);
                    let pageurl = global_var.current_page.url();
                    ...
                    try {
                            ...
                            for (let bt = 0; bt <= 5; bt++) {
                                try {
                                    await global_var.current_page
                                        .waitForSelector(`.side-toolbar__action.forward`)
                                        .then(async (el) => {
                                            await el.click();
                                        });
                                    msg_box = await global_var.current_page.waitForSelector(`.bili-rich-textarea`);
                                    await msg_box.focus();
                                    let msg_box_content = await my_operator.basic_operator.get_opus_dynamic_repost_area_content(msg_box);
                                    let _bt = 0;
                                    //#region 输入转发内容
                                    while (repost_content && !msg_box_content.includes(repost_content)//回复栏里的东西等于回复内容时break
                                        ) {
                                        msg_box = await global_var.current_page.waitForSelector(`.bili-rich-textarea`);
                                        await msg_box.focus();
                                        ...
                                        if (!msg_box_content.includes(repost_content)) {
                                            //如果不等就删掉重新输入，如果是转发 被转发的动态,则只需要判断是否包含即可
                                            await global_var.current_page.mouse.click(10, 10);
                                            await sleep(3e3);
                                        ...
                                        if (_bt >= 5) {
                                            console.error("转发框里输入内容失败");
                                            await my_operator.log_record.my_throw("动态评论失败");
                                            throw Error(`动态评论失败`);
                                        }
                                        _bt += 1;
                                    }
                                    //#endregion
                                    await global_var.current_page
                                        .waitForSelector(`.bili-dyn-share-publishing__action.launcher`)
                                        .then(async (el) => await el.click());
                                    console.log(`${global_var.user_info.uname}\t${pageurl}\t动态转发成功`);
                                    break;
                                } catch (e) {
                                    if (bt >= 5) {
                                        throw Error(e);
                                    }
                                    // await global_var.current_page_info.page.evaluate(() => {
                                    // 	this.scrollTo(0, 1500);
                                    // });
                                    // await global_var.current_page_info.page.evaluate(() => {
                                    // 	this.scrollTo(0, -1500);
                                    // });
                                    console.error(`${global_var.user_info.uname}\t${global_var.current_page.url()} 模拟点击转发失败！dynamic_repost，${e}\n${e.stack}`);
                        ...
                        console.error(`${global_var.user_info.uname}\t${global_var.current_page.url()} 动态转发失败，dynamic_repost，${e}\n${e.stack}`);
                        await my_operator.log_record.my_throw(`动态转发失败，dynamic_repost，${e}`);
                        //return
                    }
                }, /**
                 * 点击回复按钮
                ...
                comment_submit: async (comment_msg, opus_dynamic = false) => {
                    ...
                    async function CheckRisk() {
                        ...
                        let captcha; //检查验证码
                        try {
                            captcha = await global_var.current_page.$(`.comment-captcha`);
                        } catch (e) {
                        ...
                        if (captcha || comment_dyn_response_code) {
                            await my_operator.log_record.my_throw("动态评论失败，需要验证码");
                            console.warn(`${global_var.user_info.uname}\t动态${await global_var.current_page.url()} 评论失败，需要验证码，休眠4小时！\t${new Date().toLocaleTimeString()}`);
                    ...
                    if (pageurl.includes("read/cv")) {
                        opus_dynamic = false;
                        await global_var.current_page.goto(`https://t.bilibili.com/${global_var.dynamic_id}`);
                    }
                    ...
                    if (typeof comment_msg != "string" || !comment_msg || comment_msg.includes("undefined") || comment_msg.includes("null") || comment_msg.includes("true") || comment_msg.includes("false")) {
                        ...
                        return await my_operator.log_record.my_throw("动态评论内容出错");
                    }
                    await sleep(1e3);
                    ...
                    for (let i = 0; i < 3; i++) {
                        ...
                        try {
                            if (opus_dynamic || 1) {
                                let msg_box = await global_var.current_page.waitForSelector(`.reply-box-textarea`, {timeout: 30e3});
                                await msg_box.click();
                                let msg_box_content = await global_var.current_page.$eval(`.reply-box-textarea`, (el) => el.value);
                                let _bt = 0;
                                while (msg_box_content != comment_msg) {
                                    ...
                                    await sleep(1e3);
                                    msg_box_content = await global_var.current_page.$eval(`.reply-box-textarea`, (el) => el.value);
                                    if (utl.remove_invisible_char(msg_box_content.replaceAll(/[\uD800-\uDBFF][\uDC00-\uDFFF]/g, "")) != utl.remove_invisible_char(comment_msg.replaceAll(/[\uD800-\uDBFF][\uDC00-\uDFFF]/g, ""))) {
                                        //如果不等就删掉重新输入
                                        await sleep(1e3);
                                        await msg_box.click();
                                        await global_var.current_page.keyboard.down("Control");
                                        await global_var.current_page.keyboard.press("A");
                                        await global_var.current_page.keyboard.up("Control");
                                        await sleep(1e3);
                                        await global_var.current_page.keyboard.press("Backspace");
                                ...
                                await sleep(1e3);
                                global_var.current_page.click(`.send-text`);
                                await MYAPI.PageFunc.waitForResponse(global_var.current_page, "reply/add").catch((e) => {
                                    throw Error(e);
                                });
                                await sleep(1e3);
                            ...
                            bt++;
                            console.error(global_var.user_info.uname, global_var.current_page.url(), `第${i + 1}次尝试输入动态评论！`, e);
                    ...
                    for (let i = 0; i < 10; i++) {
                        if (global_var.response.comment_dyn_response) {
                            ...
                            if (check_reply_result) {
                                console.log(`${global_var.user_info.uname}\t${global_var.current_page.url()}\t评论成功，躲过阿瓦隆\t${new Date().toLocaleString()}`);
                                break;
                            } else {
                                console.error(`${global_var.user_info.uname}\t${global_var.current_page.url()}\t动态评论失败，评论被隐藏\t${new Date().toLocaleString()}`);
                                await my_operator.log_record.my_throw("动态评论失败，评论被隐藏");
                                break;
                            }
                        } else {
                            if (i == 9) {
                                await my_operator.log_record.my_throw(`动态评论失败，获取评论响应失败${new Date().toLocaleString()}`);
                            }
                        }
                        await sleep(2e3);
                    }
                    ...
                    if (opus_dynamic) {
                        try {
                            ...
                            await sleep(3e3);
                            let comment_user_index = await global_var.current_page.$$eval(`.user-name`, (els, uname) => {
                            ...
                            try {
                                my_comment_thumb = (await global_var.current_page.$$(`.reply-like`))[comment_user_index];
                            } catch (e) {
                            ...
                            if (my_comment_thumb) {
                                ...
                                return await my_operator.log_record.my_throw("评论点赞失败");
                            }
                            if (!(await global_var.current_page.waitForSelector(`.svg-icon.liked.use-color.like-icon.liked`, {timeout: 10e3}))) {
                                console.warn("评论点赞失败");
                                return await my_operator.log_record.my_throw("评论点赞失败");
                            } else {
                                console.log("评论点赞成功");
                            ...
                            await my_operator.log_record.my_throw(`评论点赞失败，comment_thumb，${e}`);
            ...
            video_operator: {
                goto_video_page: async function (pageurl) {
                    await global_var.current_page.goto(pageurl);
                    await global_var.current_page.waitForSelector(`.bpx-player-video-area`);
                    await pptr_op.remove_video_player(global_var.current_page);
                }, sanlian: async function (pageurl) {
                    await sleep(10e3);
                    await global_var.current_page.evaluate(() => {
                        this.scrollTo(0, 1500);
                    });
                    await global_var.current_page.keyboard.press("q", {delay: 10e3});
                    // let thumb_btn = await global_var.current_page_info.page
                    ...
                    // 	});
                    // let coin_btn = await global_var.current_page_info.page.$(
                    // 	`.video-coin.video-toolbar-left-item`
                    // );
                    let coin_btn_On = await global_var.current_page
                        .waitForSelector(`.video-coin.video-toolbar-left-item.on`)
                        .catch((e) => {
                    ...
                    let coin_btn = await global_var.current_page
                    ...
                    if (coin_num == 1) {
                        let one_coin_box = await global_var.current_page
                            .waitForSelector(`.mc-box.left-con`)
                            .then(async (el) => await el.click());
                    }
                    let coin_confirm_btn = await global_var.current_page
                ...
                try {
                    ...
                    try {
                        await sleep(utl.random_choice(lottery_setting.Working_clearance_time));
                        await global_var.current_page.click(".bili-dyn-action.forward"); //前往转发子页面
                        await sleep(1e3);
                        await my_operator.basic_operator.dynamic_repost(opus_dynamic);
                        //最后点赞
                        await my_operator.basic_operator.dynamic_thumb(opus_dynamic);
                        ...
                        global_var.fengkong_flag = true; //可能触发风控，停一个小时
                        return await my_operator.log_record.my_throw(`转发失败，fast_repost`);
                    }
                }
            }, /**
             * 勾选同时转发到我的动态
             ...
             * @returns
             */
            comment_repost_dynamic_with_content: async (comment_msg, opus_dynamic = false) => {
                //转评带上回复内容
                let pageurl = await global_var.current_page.url();
                if (pageurl.includes("opus")) {
                    opus_dynamic = true;
                ...
                if (opus_dynamic) {
                    try {
                        ...
                        return await my_operator.log_record.my_throw(`转发失败，comment_repost_dynamic_with_content，${e}\n${pageurl}\t${global_var.user_info.uname}`);
                    }
                } else {
                    try {
                        // let bt = 0;
                        ...
                        // 			`.reply-box-textarea`
                        // 		);
                        // 		await global_var.current_page_info.page
                        ...
                        // 			}); //勾选同时转发到我的动态
                        // 		//  await global_var.current_page_info.page.click('.dynamic-repost-checkbox')
                        // 		await sleep(1e3);
                        // 		if (
                        // 			await global_var.current_page_info.page.$eval(
                        // 				".forward-input",
                        // 				(el) => el.checked
                        // 			)
                        // 		) {
                        ...
                        // 	} catch (e) {
                        // 		await sleep(1e3);
                        // 		await global_var.current_page_info.page.reload();
                        // 		await sleep(3e3);
                        // 		await global_var.current_page_info.page.evaluate(() => {
                        // 			this.scrollTo(0, 1500);
                        // 		});
                        ...
                        // } else {
                        // 	console.log(
                        // 		`勾选同时转发到我的动态转发失败\t${global_var.current_page_info.page.url()}\t${global_var.user_info.uname}`
                        // 	);
                        ...
                        if (comment_msg != null) {
                            ...
                            console.error(`评论获取失败，comment_repost_dynamic_with_content\t${global_var.current_page.url()}\t${global_var.user_info.uname}`);
                            return await my_operator.log_record.my_throw("评论获取失败，comment_repost_dynamic_with_content");
                        }
                        //检查转发是否成功
                        try {
                            let bt = 0;
                            ...
                            //         global_var.fengkong_flag = true//可能触发风控，停一个小时
                            //         return await my_operator.log_record.my_throw(`动态转发失败，comment_repost_dynamic_with_content`)
                            //     }
                            // }
                            // else {
                            //     console.log('动态转发成功');
                            ...
                            global_var.fengkong_flag = true; //可能触发风控，停一个小时
                            await my_operator.log_record.my_throw(`动态转发失败，comment_repost_dynamic_with_content，${e}`);
                            throw Error(`动态转发失败，comment_repost_dynamic_with_content，${e}\n${global_var.current_page.url()}\t${global_var.user_info.uname}`);
                        }
             ...
             * @returns
             */
            comment_repost_dynamic_without_content: async (comment_msg, opus_dynamic) => {
                //转评不带回复内容
                let pageurl = await global_var.current_page.url();
                if (pageurl.includes("opus")) {
                    opus_dynamic = true;
                ...
                if (opus_dynamic) {
                    try {
                        ...
                        return await my_operator.log_record.my_throw(`评论转发失败，comment_repost_dynamic_without_content，${e}`);
                    }
                } else {
                    //先评论
                    try {
                        if (comment_msg != null && comment_msg != undefined) {
                            ...
                            return await my_operator.log_record.my_throw("评论获取失败， comment_repost_dynamic_without_content");
                        }
                        //再转发
                        await sleep(1e3);
                        // await global_var.current_page_info.page.click(".bili-dyn-action.forward"); //前往转发子页面
                        // await sleep(1e3);
                        await my_operator.basic_operator.dynamic_repost(opus_dynamic);
                        //最后点赞
                        await my_operator.basic_operator.dynamic_thumb(opus_dynamic);
                ...
                if (opus_dynamic) {
                    try {
                        ...
                        console.error(`评论获取失败\n${JSON.stringify(global_var.response.global_dynamic_data)}\t${pageurl}\t${global_var.user_info.uname}`, e);
                        return await my_operator.log_record.my_throw(`评论获取失败， only_comment，${e}`);
                    }
                } else {
                    try {
                        if (comment_msg != null && comment_msg != undefined) {
                        ...
                        console.warn(`评论获取失败\n${JSON.stringify(global_var.response.global_dynamic_data)}\n${pageurl}\n${global_var.user_info.uname}`);
                        return await my_operator.log_record.my_throw(`评论获取失败， only_comment，${e}`);
                    }
                }
                    ...
                    let get_top_msg = async () => {
                        try {
                            if (global_var.response.reply_main != undefined) {
                                try {
                                    if (global_var.response.reply_main.code == 12061 || global_var.response.reply_main.code == 12002 || global_var.response.reply_main?.data?.control?.input_disable //无法评论
                                    ) {
                                        // code:
                                        // 12061
                                        // message:
                                    ...
                                    console.error(`up置顶的回复获取失败`, e, global_var.response.reply_main);
                                    return "";
                                }
                            } else {
                                console.log("未拦截到评论API内容");
                                await my_operator.log_record.my_throw("获取置顶评论失败");
                                return "";
                            }
                        } catch (e) {
                            console.error(`up置顶的回复获取失败`, e);
                            await my_operator.log_record.my_throw("up置顶的回复获取失败");
                            return "";
                        }
                    };
                    try {
                        if (!dynamic_data) {
                            dynamic_data = (await MYAPI.BiliAPI.get_dynamic_v1_detail(MYAPI.BiliAPI.draw_dynamic_id(global_var.current_page.url()))).data;
                        ...
                        if (dynamic_type == "DYNAMIC_TYPE_AV") {
                            ...
                            if (dynamic_content1 != undefined && dynamic_content1 != null) {
                                dynmaic_content += dynamic_content1;
                            }
                            if (dynamic_content2 != undefined && dynamic_content2 != null) {
                                dynmaic_content += dynamic_content2;
                            }
                            if (dynamic_content3 != undefined && dynamic_content3 != null) {
                            ...
                            if (dynamic_content4 != undefined && dynamic_content4 != null) {
                            ...
                            let dynamic_content4;
                            try {
                                dynamic_content1 = dynamic_data.item.modules.module_dynamic.major?.opus?.summary?.rich_text_nodes
                                    ?.map((el) => el.text)
                                    .join("");
                            } catch {
                                dynamic_content1 = "";
                            ...
                            try {
                                dynamic_content4 = dynamic_data.item.modules.module_dynamic?.major?.opus?.title;
                            } catch {
                                dynamic_content4 = "";
                            }
                            try {
                        ...
                        let ret_dynamic_content = (dynmaic_content + "\n" + String(top_msg).replaceAll("undefined", "")).trim();
                ...
                pre_msg_processing: function (dynamic_content, reply_msg) {
                    ...
                    dynamic_content = dynamic_content.replaceAll(/@((?! ).){1,10} /gim, "");
                    dynamic_content = dynamic_content.replaceAll(/标记/gim, "艾特");
                    dynamic_content = dynamic_content.replaceAll(/朋友/gim, "好友");
                    ...
                    let non_topic_content = dynamic_content.replaceAll(/(?<=#)(.{0,10})(?=#)/gim, "");
                    let topobj_6 = non_topic_content.match(/@.{0,3}位.*|.*@.{0,3}名.*/gim);
                    let topobj_5 = non_topic_content.match(/@.{0,3}1位.*|.*@.{0,3}1名.*/);
                    let topobj_4 = non_topic_content.match(/@.{0,3}一位.*|.*@.{0,3}一名.*/gim);
                    let topobj_3 = non_topic_content.match(/@.{0,3}一位好友.*|.*@.{0,3}你的|.*@.{0,3}一名好友.*/gim);
                    let topobj_2 = non_topic_content.match(/艾特.{0,3}位好友.*|.*艾特.{0,3}名好友.*|艾特.{0,7}up/gim);
                    let topobj_1 = non_topic_content.match(/@你想祝福的人.*/gim);
                    let topobj0 = non_topic_content.match(/@{0,3}位胖友.*|.*@{0,3}名胖友.*/gim);
                    let topobj1 = non_topic_content.match(/圈.{0,3}位你的伙伴.*|.*圈.{0,3}名你的伙伴.*/gim);
                    let topobj2 = non_topic_content.match(/带tag#.{0,30}#.*/gim);
                    let topobj3 = non_topic_content.match(/带话题.{0,40}#.{0,30}#((?!投稿).)*$/gim);
                    let topobj4 = non_topic_content.match(/带上tag#.{0,30}#((?!投稿).)*$/gim);
                    let topobj5 = non_topic_content.match(/带#.{0,30}#.{0,10}话题((?!投稿).)*$/gim);
                    let topobj6 = non_topic_content.match(/艾特好友.*/gim);
                    let topobj7 = non_topic_content.match(/@.{0,4}名好友.*|.*@.{0,4}位好友.*/gim);
                    let topobj8 = non_topic_content.match(/@你的.{0,3}个小伙伴.*/gim);
                    let topobj9 = non_topic_content.match(/@两位好友.*|.*@两名好友.*/gim);
                    let topobj10 = non_topic_content.match(/带#.{0,30}#((?!投稿).)*$/gim);
                    let topobj11 = non_topic_content.match(/@.{0,5}你的.{0,3}个好友.*/gim);
                    let topobj12 = non_topic_content.match(/带[^来】看懂]{0,5}#.{0,30}#((?!投稿).)*$/gim);
                    let topobj13 = non_topic_content.match(/加话题#.{0,30}#((?!投稿).)*$/gim);
                    let topobj14 = non_topic_content.match(/带标签#.{0,30}#((?!投稿).)*$/gim);
                    let topobj15 = non_topic_content.match(/@三位好友.*|.*@三名好友.*/gim);
                    let topobj_16 = non_topic_content.match(/带(.{0,3}#.{0,20}) 话题.(?!投稿).*?/gim);
                    if (topobj_6 != null || topobj6 != null || topobj_5 != null || topobj_4 != null || topobj_3 != null || topobj_2 != null || topobj_1 != null || topobj0 != null || topobj1 != null || topobj7 != null || topobj8 != null || topobj11 != null) {
                        let UPname = "";
                        try {
                        ...
                        let at_times = 1;
                        let findContent = [topobj_6, topobj6, topobj_5, topobj_4, topobj_3, topobj_2, topobj_1, topobj0, topobj1, topobj7, topobj8, topobj11,].join("");
                        let num = parseInt(findContent.match(/\d+/gim).join("") || zhDigitToArabic(findContent));
                        if (num > 0 && num < 5) {
                            at_times = num;
                        }
                        let choose_Up_list = [];
                        premsg = "@" + (UPname ? UPname : utl.random_choice(lottery_setting.at_member)) + " ";
                        for (let i = 0; i < at_times - 1; i++) {
                            let at_up = "";
                            while (!choose_Up_list.includes(at_up)) {
                                at_up = utl.random_choice(lottery_setting.at_member);
                                if (!choose_Up_list.includes(at_up)) {
                                    choose_Up_list.push(at_up);
                                }
                                if (choose_Up_list.length == lottery_setting.at_member.length) break;
                            }
                            premsg += "@" + at_up + " ";
                        }
                    } else if (topobj9 != null) {
                        premsg = `@${utl.random_choice(lottery_setting.at_member)} @${utl.random_choice(lottery_setting.at_member)} `;
                    } else if (topobj2 != null) {
                        msg = /带tag#(.{0,20})#.*/gim
                            .exec(dynamic_content)
                            ?.slice(1);
                        if (msg) for (let _ = 0; _ < msg.length; _++) {
                    ...
                    if (/带话题#.*#((?!投稿).)*$/gim.test(non_topic_content) || /带((?!】|来|看懂)).{0,5}#/.test(non_topic_content) || topobj2 || topobj3 || topobj4 || topobj5 || topobj10 || topobj12 || topobj13 || topobj14) {
                        if (!(premsg.includes("#") || reply_msg.includes("#"))) {
                            // my_operator.log_record.my_throw("话题获取失败");
                            return undefined;
                        }
                    }
                    return premsg;
                ...
                manual_reply_judge: function (dynamic_content) {
                    ...
                    dynamic_content = dynamic_content.replaceAll(/[\?|❓]/gim, "？");
                    dynamic_content = dynamic_content.replaceAll(/:/gim, "：");
                    let manual_re1 = /.*评论.{0,20}告诉|.*有关的评论|.*告诉.{0,20}留言/gim.test(dynamic_content);
                    let manual_re2 = /.*评论.{0,20}理由|.*参与投稿.{0,30}有机会获得/gim.test(dynamic_content);
                    let manual_re3 = /.*评论.{0,10}对|.*造.{0,3}句子/gim.test(dynamic_content);
                    let manual_re4 = /.*猜赢|.*猜对|.*答对|.*猜到.{0,5}答案/gim.test(dynamic_content);
                    let manual_re5 = /.*说.{0,10}说|.*谈.{0,10}谈|.*夸.{0,10}夸|评论.{0,10}写.{0,10}写|.*写下.{0,5}假如.{0,5}是|.*讨论.{0,10}怎么.{0,10}？/gim.test(dynamic_content);
                    let manual_re7 = /.*最先猜中|.*带文案|.*许.{0,5}愿望/gim.test(dynamic_content);
                    let manual_re8 = /.*新衣回/gim.test(dynamic_content);
                    let manual_re9 = /.*留言.{0,10}建议|.*评论.{0,10}答|.*一句话证明|.*留言.{0,10}得分|.*有趣.{0,3}留言|.*有趣.{0,3}评论|.*留言.{0,3}晒出|.*评论.{0,3}晒出/gim.test(dynamic_content);
                    let manual_re11 = /.*评论.{0,10}祝福|.*评论.{0,10}意见|.*意见.{0,10}评论|.*留下.{0,10}意见|.*留下.{0,15}印象|.*意见.{0,10}留下/gim.test(dynamic_content);
                    let manual_re12 = /.*评论.{0,10}讨论|.*话题.{0,10}讨论|.*参与.{0,5}讨论/gim.test(dynamic_content);
                    let manual_re14 = /.*评论.{0,10}说出|,*留言.{0,5}身高/gim.test(dynamic_content);
                    let manual_re15 = /.*评论.{0,20}分享|.*评论.{0,20}互动((?!抽奖|,|，|来).)*$|.*评论.{0,20}提问|.*想问.{0,20}评论|.*想说.{0,20}评论|.*想问.{0,20}留言|.*想说.{0,20}留言/gim.test(dynamic_content);
                    let manual_re16 = /.*评论.{0,10}聊.{0,10}聊/gim.test(dynamic_content);
                    let manual_re17 = /.*评.{0,10}接力/gim.test(dynamic_content);
                    let manual_re18 = /.*聊.{0,10}聊|有没有.{0,20}事.{0,5}？/gim.test(dynamic_content);
                    let manual_re19 = /.*评论.{0,10}扣|.*评论.{0,5}说.{0,3}下/gim.test(dynamic_content);
                    let manual_re20 = /.*转发.{0,10}分享/gim.test(dynamic_content);
                    let manual_re21 = /.*评论.{0,10}告诉/gim.test(dynamic_content);
                    let manual_re22 = /.*评论.{0,10}唠.{0,10}唠/gim.test(dynamic_content);
                    let manual_re23 = /.*今日.{0,5}话题|.*参与.{0,5}话题|.*参与.{0,5}答题/gim.test(dynamic_content);
                    let manual_re24 = /.*说.*答案|.*评论.{0,15}答案/gim.test(dynamic_content);
                    let manual_re25 = /.*说出/gim.test(dynamic_content);
                    let manual_re26 = /.*为.{0,10}加油/gim.test(dynamic_content);
                    let manual_re27 = /.*评论.{0,10}话|.*你中意的|.*评.{0,10}你.{0,5}的|.*写上.{0,10}你.{0,5}的|.*写下.{0,10}你.{0,5}的/gim.test(dynamic_content);
                    let manual_re28 = /.*评论.{0,15}最想做7的事|.*评.{0,15}最喜欢|.*评.{0,15}最.{0,7}的事|.*最想定制的画面|最想.{0,20}\?|最想.{0,20}？/gim.test(dynamic_content);
                    let manual_re29 = /.*分享.{0,20}经历|.*经历.{0,20}分享/gim.test(dynamic_content);
                    let manual_re30 = /.*分享.{0,20}心情/gim.test(dynamic_content);
                    let manual_re31 = /.*评论.{0,10}句|评论.{0,6}包含/gim.test(dynamic_content);
                    let manual_re32 = /.*转关评下方视频/gim.test(dynamic_content);
                    let manual_re33 = /.*分享.{0,10}美好|.*分享.{0,10}期待/gim.test(dynamic_content);
                    let manual_re34 = /.*视频.{0,10}弹幕/gim.test(dynamic_content);
                    let manual_re35 = /.*生日快乐/gim.test(dynamic_content);
                    let manual_re36 = /.*一句话形容/gim.test(dynamic_content);
                    let manual_re38 = /.*分享.{0,10}喜爱|.*分享.{0,10}最爱|.*推荐.{0,10}最爱|.*推荐.{0,10}喜爱/gim.test(dynamic_content);
                    let manual_re39 = /.*分享((?!,|，).){0,10}最|.*评论((?!,|，).){0,10}最/gim.test(dynamic_content);
                    let manual_re40 = /.*带话题.{0,15}晒|.*带话题.{0,15}讨论/gim.test(dynamic_content);
                    let manual_re41 = /.*分享.{0,15}事|点赞.{0,3}数.{0,3}前/gim.test(dynamic_content);
                    let manual_re42 = /.*送出.{0,15}祝福/gim.test(dynamic_content);
                    let manual_re43 = /.*评论.{0,30}原因/gim.test(dynamic_content);
                    let manual_re47 = /.*答案.{0,10}参与/gim.test(dynamic_content);
                    let manual_re48 = /.*唠.{0,5}唠/gim.test(dynamic_content);
                    let manual_re49 = /.*分享一下/gim.test(dynamic_content);
                    let manual_re50 = /.*评论.{0,30}故事/gim.test(dynamic_content);
                    let manual_re51 = /.*告诉.{0,30}什么|.*告诉.{0,30}最|有什么安排呀～/gim.test(dynamic_content);
                    let manual_re53 = /.*发布.{0,20}图.{0,5}动态/gim.test(dynamic_content);
                    let manual_re54 = /.*视频.{0,20}评论/gim.test(dynamic_content);
                    let manual_re55 = /.*复zhi|.*长按/gim.test(dynamic_content);
                    let manual_re56 = /.*多少.{0,10}合适/gim.test(dynamic_content);
                    let manual_re57 = /.*喜欢.{0,5}哪/gim.test(dynamic_content);
                    let manual_re58 = /.*多少.{0,15}？|.*多少.{0,15}\?|.*有没有.{0,15}？|.*有没有.{0,15}\?|.*是什么.{0,15}？|.*是什么.{0,15}\?/gim.test(dynamic_content);
                    let manual_re61 = /.*看.{0,10}猜/gim.test(dynamic_content);
                    let manual_re63 = /.*评论.{0,10}猜|.*评论.{0,15}预测|选择.{0,5}任意.{0,17}评论/gim.test(dynamic_content);
                    let manual_re65 = /.*老规矩你们懂的/gim.test(dynamic_content);
                    let manual_re67 = /.*[评|带]((?!抽奖|,|，|来).){0,7}“|.*[评|带]((?!抽奖|,|，|来).){0,7}"|.*[评|带]((?!抽奖|,|，|来).){0,7}【|.*[评|带]((?!抽奖|,|，|来).){0,7}：|.*[评|带]((?!抽奖|,|，|来).){0,7}：|.*[评|带]((?!抽奖|,|，|来).){0,7}「|.*带关键词.{0,7}"|.*评论关键词[“”‘’"']|.*留言((?!抽奖|,|，|来).){0,7}“|.*对出.{0,10}下联.{0,5}横批|.*回答.{0,8}问题|.*留下.{0,10}祝福语|.*留下.{0,10}愿望|.*找到.{0,10}不同的.{0,10}留言|.*答案放在评论区|.*几.{0,5}呢？|.*有奖问答|.*想到.{0,19}关于.{0,20}告诉|.*麻烦大伙评论这个|报暗号【.{0,4}】|评论.{0,3}输入.{0,3}["“”:：]|.*评论.{0,7}暗号/gim.test(dynamic_content);
                    let manual_re76 = /.*留言((?!抽奖|,|，|来).).{0,7}"|.*留下((?!抽奖|,|，|来).){0,5}“|.*留下((?!抽奖|,|，|来).){0,5}【|.*留下((?!抽奖|,|，|来).){0,5}：|.*留下((?!抽奖|,|，|来).){0,5}「/gim.test(dynamic_content);
                    let manual_re77 = /.*留言((?!抽奖|,|，|来).).{0,7}"|.*留言((?!抽奖|,|，|来).).{0,7}“|.*留言((?!抽奖|,|，|来).){0,7}【|.*留言((?!抽奖|,|，|来).){0,7}：|.*留言((?!抽奖|,|，|来).){0,7}「/gim.test(dynamic_content);
                    let manual_re64 = /和.{0,5}分享.{0,5}的|.*分享.{0,10}你的|.*正确回答|.*回答正确|.*评论.{0,10}计划|.*定.{0,10}目标.{0,5}？|.*定.{0,10}目标.{0,5}?|.*评论.{0,7}看的电影|.*如果.{0,20}觉得.{0,10}？|.*如果.{0,20}觉得.{0,10}\?|评论.{0,7}希望.{0,5}|.*竞猜[\s\S]{0,15}[答评]|.*把喜欢的.{0,10}评论|.*评论.{0,5}解.{0,5}密|.*这款.{0,10}怎么.{0,3}？|.*最喜欢.{0,5}的.*为什么？|.*留下.{0,15}的.{0,5}疑问|.*写下.{0,10}的.{0,5}问题/gim.test(dynamic_content);
                    let manual_re6 = /.*@TA|.*@.{0,15}朋友|.*艾特|.*@.{0,3}你的|.*标记.{0,10}朋友|.*@{0,15}赞助商|.*发表你的新年愿望\+个人的昵称|.*抽奖规则请仔细看图片|.*带上用户名|.*活动详情请戳图片|.*@个人用户名|评论.{0,5}附带.{0,10}相关内容|回复.{0,5}视频.{0,10}相关内容|.*评论.{0,5}昵称|转发.{0,8}并@/gim.test(dynamic_content);
                    let manual_re62 = /.*评论.{0,10}#.*什么|.*转评.{0,3}#.*(?<=，)/gim.test(dynamic_content);
                    let manual_re68 = /.*将.{0,10}内容.{0,10}评|.*打几分？/gim.test(dynamic_content);
                    let manual_re70 = /.*会不会.{0,20}？|.*会不会.{0,20}\?|如何.{0,20}？|如何.{0,20}\?/gim.test(dynamic_content);
                    let manual_re71 = /.*猜.{0,10}猜|.*猜.{0,10}比分|.*猜中.{0,10}获得|.*猜中.{0,10}送出/gim.test(dynamic_content);
                    let manual_re72 = /.*生日|.*新年祝福/gim.test(dynamic_content);
                    let manual_re73 = /.*知道.{0,15}什么.{0,15}？|.*知道.{0,15}什么.{0,15}\?|.*用什么|.*评.{0,10}收.{0,5}什么.{0.7}\?|.*评.{0,10}收.{0,5}什么.{0,7}？|.*抽奖口令.{0,3}：/gim.test(dynamic_content);
                    let manual_re74 = /.*领.{0,10}红包.{0,5}大小|.*领.{0,10}多少.{0,10}红包|.*红包金额/gim.test(dynamic_content);
                    let manual_re75 = /.*本周话题|.*互动话题|.*互动留言|.*互动时间|.*征集.{0,10}名字|.*征集.{0,15}外号|.*投票.{0,5}选.{0,10}最.{0,5}的|.*投票.{0,10}评论|.*一人说一个谐音梗|帮.{0,5}想想.{0,5}怎么|评论.{0,5}想给.{0,7}的|取.{0,7}名字/gim.test(dynamic_content);

                    return (manual_re1 || manual_re2 || manual_re3 || manual_re4 || manual_re5 || manual_re6 || manual_re7 || manual_re8 || manual_re9 || manual_re11 || manual_re12 || manual_re14 || manual_re15 || manual_re16 || manual_re17 || manual_re18 || manual_re19 || manual_re20 || manual_re21 || manual_re22 || manual_re23 || manual_re24 || manual_re25 || manual_re26 || manual_re27 || manual_re28 || manual_re29 || manual_re30 || manual_re31 || manual_re32 || manual_re33 || manual_re34 || manual_re35 || manual_re36 || manual_re38 || manual_re39 || manual_re40 || manual_re41 || manual_re42 || manual_re43 || manual_re76 || manual_re47 || manual_re48 || manual_re49 || manual_re50 || manual_re51 || manual_re53 || manual_re54 || manual_re58 || manual_re55 || manual_re56 || manual_re57 || manual_re61 || manual_re62 || manual_re63 || manual_re64 || manual_re65 || manual_re67 || manual_re68 || manual_re70 || manual_re71 || manual_re72 || manual_re73 || manual_re74 || manual_re75 || manual_re77 || manual_re77);
                }, /**
                 * 返回true代表这个动态不是抽奖up的动态，不能转发评论
                 */
                non_lottery_up_judge: () => {
                    try {
                        let non_lottery_up_mids = GLOBAL_CONFIG.lot_module.non_lottery_up_mids ? GLOBAL_CONFIG.lot_module.non_lottery_up_mids : ["571791768", "391464745", "14064125", "332793152", "54790268", "46880349", //"294887687",
                            "3493120108923438", "3537106980833281", "3532811", "1508263674",];
                    ...
                    if (/.*[评|带]((?!抽奖|,|，|来).){0,7}“|.*[评|带]((?!抽奖|,|，|来).){0,7}"|.*[评|带]((?!抽奖|,|，|来).){0,7}【|.*[评|带]((?!抽奖|,|，|来).){0,7}:|.*[评|带]((?!抽奖|,|，|来).){0,7}：|.*[评|带]((?!抽奖|,|，|来).){0,7}「|.*带关键词.{0,7}"|.*评论关键词[“”‘’"']|.*留言((?!抽奖|,|，|来).){0,7}“|.*对出.{0,10}下联.{0,5}横批|.*回答.{0,8}问题|.*留下.{0,10}祝福语|.*留下.{0,10}愿望|.*找到.{0,10}不同的.{0,10}留言|.*答案放在评论区|.*几.{0,5}呢？|.*有奖问答|.*想到.{0,19}关于.{0,20}告诉|.*麻烦大伙评论这个|留下.{0,7}的|报暗号【.{0,4}】/gim.test(dynamic_content) || /.*留下((?!抽奖|,|，).){0,5}“|.*留下((?!抽奖|,|，).){0,5}【|.*留下((?!抽奖|,|，).){0,5}:|.*留下((?!抽奖|,|，).){0,5}：|.*留下((?!抽奖|,|，).){0,5}「/gim.test(dynamic_content) || /.*留言((?!抽奖|,|，).).{0,7}“|.*留言((?!抽奖|,|，).){0,7}【|.*留言((?!抽奖|,|，).){0,7}:|.*留言((?!抽奖|,|，).){0,7}：|.*留言((?!抽奖|,|，).){0,7}「/gim.test(dynamic_content) || /.*分享.{0,10}你的|.*正确回答|.*回答正确|.*评论.{0,10}计划|.*定.{0,10}目标.{0,5}？|.*定.{0,10}目标.{0,5}?|.*评论.{0,7}看的电影|.*如果.{0,20}觉得.{0,10}？|.*如果.{0,20}觉得.{0,10}\?|评论.{0,7}希望.{0,5}|.*竞猜[\s\S]{0,15}答|.*把喜欢的.{0,10}评论|.*评论.{0,5}解.{0,5}密|.*这款.{0,10}怎么.{0,3}？|.*最喜欢.{0,5}的.*为什么？|.*留下.{0,15}的.{0,5}疑问|.*写下.{0,10}的.{0,5}问题/gim.test(dynamic_content) || /.*@TA|.*@.{0,15}朋友|.*艾特|.*@.{0,3}你的|.*标记.{0,10}朋友|.*@{0,15}赞助商|.*发表你的新年愿望\+个人的昵称|.*抽奖规则请仔细看图片|.*带上用户名|.*活动详情请戳图片|.*@个人用户名|评论.{0,5}附带.{0,10}相关内容|回复.{0,5}视频.{0,10}相关内容/gim.test(dynamic_content) || /.*评论.{0,10}#.*什么|.*转评.{0,3}#.*(?<=，)/gim.test(dynamic_content) || /.*本周话题|.*互动话题|.*互动留言|.*互动时间|.*征集.{0,10}名字/gim.test(dynamic_content)) {
                        //如果是指定回复某个评论直接返回undefined
                        return undefined;
                    }
                    if (/.*留下((?!抽奖).){0,5}“|.*留下((?!抽奖).){0,5}【|.*留下((?!抽奖).){0,5}:|.*留下((?!抽奖).){0,5}：|.*留下((?!抽奖).){0,5}「/gim.test(dynamic_content)) {
                        return undefined;
                    }
                    if (/.*猜.{0,10}猜|.*猜.{0,10}比分|.*猜中.{0,10}获得/gim.test(dynamic_content)) {
                        return undefined;
                    }
                    if (/.*领到多少红包|.*领.{0,3}到.{0,3}红包大小|.*评论.{0,10}红包金额|留言.{0,10}红包金额|.*领.{0,3}的.{0,3}红包大小/gim.test(dynamic_content)) {
                    ...
                    if (/.*喜欢.{0,5}颜色|.*最爱.{0,5}颜色/gim.test(dynamic_content)) {
                    ...
                    if (/.*生日季|.*生日回|.*生日会|.*生日祝福|.*岁生日/gim.test(dynamic_content)) {
                    ...
                    if (/.*长按.{0,5}复制|.*复制.{0,5}长按|.*长按.{0,5}fu制|.*长按.{0,5}copy/gim.test(dynamic_content)) {
                ...
                repost_with_comment_judge: (dynamic_content) => {
                    dynamic_content = my_operator.dynamic_comment_operator.pre_process_dynamic_content(dynamic_content);
                    let re_1 = /.*转发.{0,5}含关键词|转发.{0,8}并@/gim.test(dynamic_content);
                    return re_1;
                }, /**
                 ...
                 * @returns
                 */
                reply_comment_generator: async (//返回undefined表示需要人工回复，而不是从预设的回复里面选内容
                                                dynamic_content, dynamic_id) => {
                    //生成所需评论//生成评论
                    let comment_msg = undefined;
                    if (my_operator.dynamic_comment_operator.non_lottery_up_judge()) {
                        console.log("包含非抽奖up，跳过");
                        comment_msg = "人工回复";
                        await my_operator.log_record.my_throw("需要人工回复的动态");
                        return undefined;
                    }
                    let pre_msg = "";
                    pre_msg = my_operator.dynamic_comment_operator.pre_msg_processing(dynamic_content, comment_msg);
                    if (my_operator.dynamic_comment_operator.manual_reply_judge(dynamic_content) || global_var.response.global_dynamic_data.item.basic.comment_type == 1 || pre_msg == undefined) {
                        //先判断是否要人工回复 视频全部抄
                        ...
                        if (!key_reply) {
                             ...
                             */
                            let e = {prev: 0, next: 0};
                            let copy_msg_flag = global_var.response.reply_main?.code == 12061 ? false : my_operator.copy_reply_module.copy_reply_judge(dynamic_content) || global_var.response.global_dynamic_data.item.basic.comment_type == 1;
                            if (!copy_msg_flag) {
                                //如果不能抄评论，先设置为人工回复
                                e.next = 2;
                            }
                            if (copy_msg_flag && Math.random() < lottery_setting.copy_reply_module.comment_copy_chance) {
                            ...
                            if (pre_msg == undefined) {
                                //话题获取失败了，直接开抄！
                                if (0 < lottery_setting.copy_reply_module.comment_copy_chance || 0 < lottery_setting.copy_reply_module.AI_reply_chance) {
                                    e.next = 0;
                                    pre_msg = "";
                                }
                            }
                            ...
                            while (!comment_msg) {
                                ...
                                switch ((e.prev = e.next)) {
                                    case 0:
                                        ...
                                        let para_msg;
                                        try {
                                            if (global_var.response.global_dynamic_data.item.basic.comment_type == 1 || global_var.response.global_dynamic_data.item.basic.comment_type == 8 || 1 == 1) {
                                            ...
                                            console.warn(`${global_var.user_info.uname}\t获取抄评论内容失败，reply_comment_generator\n`, e);
                                        }
                                        ...
                                        if (get_comment_times >= 3) {
                                            console.warn(`${global_var.user_info.uname}\t获取评论次数${get_comment_times}超过3次\t获取评论失败！`);
                                            e.next = 99;
                                        } else {
                                            e.next = 1;
                                        }
                                    ...
                                    case 99:
                                        console.error(`${global_var.user_info.uname}\t生成评论失败！获取评论次数${get_comment_times}超过3次\t获取评论失败！`);
                                        comment_msg = "人工回复";
                                        await my_operator.log_record.my_throw("需要人工回复的动态");
                                        return undefined; //返回undefined表示需要人工回复，而不是从预设的回复里面选内容
                                }
                            ...
                            console.log(`${global_var.current_page.url()}\n触发关键词回复:${dynamic_content}`);
                    ...
                    if (!comment_msg || typeof comment_msg != "string" || pre_msg == undefined) {
                        comment_msg = "回复内容出错";
                        console.error(`${global_var.current_page.url()}\n回复内容出错:${dynamic_content}`);
                        await my_operator.log_record.my_throw("回复内容出错");
                        return undefined;
                    }
                ...
                construct_comment_record_data: async (comment_msg) => {
                    ...
                    try {
                        if (!comment_msg.includes("404动态")) {
                            dynamic_content = JSON.stringify(global_var.response.global_dynamic_data).replace(/,/g, "，");
                            dynamic_content = dynamic_content.replaceAll(/(\[(?<=\[)(.*?)(?=\])])/gim, ""); //移除表情包
                        }
                    } catch {
                    ...
                    let lottery_reply_record = `${global_var.current_page.url()}#reply${rpid} ,${comment_count},${forward_count},${JSON.stringify(comment_msg)},${ctime},${author_name},${dynamic_content},${author_homepage},${rep_dynamic_id}`;
                    if (global_var.recorded_data || global_var.recorded_data == "") {
                        if (!global_var.recorded_data.includes(global_var.current_page.url())) {
                ...
                judge_official_lottery: () => {
                    ...
                    let lot_rich_text = global_var.response.global_dynamic_data?.item?.modules?.module_dynamic?.major?.opus?.summary?.rich_text_nodes?.filter((el) => el.type == "RICH_TEXT_NODE_TYPE_LOTTERY");
                    ...
                    if (await global_var.current_page.$(".bili-dyn-upower-lottery__title.bili-ellipsis")) {
                        return true;
                    } else {
                        return false;
                    }
                ...
                prevent_filter_init: async function () {
                    try {
                        if (lottery_setting.prevent_module.share_video_switch) {
                            await my_operator.prevent_filter_module.share_video(lottery_setting.prevent_module.share_video_num, lottery_setting.prevent_module.share_video_chance, lottery_setting.prevent_module.share_copy_chance);
                        }
                    } catch (e) {
                ...
                get_video_list: async (__share_num) => {
                    let now_pageurl = global_var.current_page.url();
                    if (!now_pageurl.includes("https://www.bilibili.com")) {
                        await global_var.current_page.goto(`https://www.bilibili.com`);
                    }
                    ...
                    while (1) {
                        ...
                        let catchele = await global_var.current_page.$$eval(".bili-video-card.is-rcmd>div.bili-video-card__wrap.__scale-wrap>a[href]", (elems) => {
                        ...
                        let fresh_btn;
                        try {
                            fresh_btn = await global_var.current_page.$(".primary-btn");
                        } catch {
                            try {
                                fresh_btn = await global_var.current_page.$(".bilifont.bili-icon_caozuo_huanyihuan");
                            } catch (e) {
                ...
                share_video: async function (share_num, share_chance, copy_chance) {
                    ...
                    async function share_video_operator(pageurl) {
                        ...
                        for (let __ = 0; __ < 5; __++) {
                            try {
                                ...
                                await global_var.current_page.hover("#share-btn-outer");
                                await sleep(3e3);
                                await global_var.current_page.click(".share-btn");
                                let share_iframe; //分享的单独的iframe
                                await sleep(3e3);
                                for (let child of global_var.current_page
                                    .mainFrame()
                                    .childFrames()) {
                                ...
                                try {
                                    if (Math.random() < copy_chance) {
                                        ...
                                        if (BV) {
                                            copycontent = await my_operator.copy_reply_module.get_copy_reply(BV, 1, 0.5, true);
                                            if (copycontent) {
                                        ...
                                        if (inputstr) {
                                            for (let bt = 0; bt <= 5; bt++) {
                                                try {
                                                    await share_iframe.waitForSelector(`#editor`, {
                                                        timeout: 10e3,
                                                    });
                                                    ...
                                                    let _bt = 0;
                                                    while (msg_box_content != inputstr) {
                                                        //回复栏里的东西等于回复内容时break
                                                        await msg_box.focus();
                                                        await sleep(utl.random_choice(3 * lottery_setting.Working_clearance_time));
                                                        await msg_box.type(inputstr, {
                                                            delay: 20,
                                                        });
                                                        await sleep(1e3);
                                                        ...
                                                        await sleep(1e3);
                                                        if (utl.remove_invisible_char(msg_box_content.replaceAll(/[\uD800-\uDBFF][\uDC00-\uDFFF]/g, "")) != utl.remove_invisible_char(inputstr.replaceAll(/[\uD800-\uDBFF][\uDC00-\uDFFF]/g, ""))) {
                                                            //如果不等就删掉重新输入
                                                            await msg_box.focus();
                                                            await global_var.current_page.keyboard.down("Control");
                                                            await global_var.current_page.keyboard.press("A");
                                    ...
                                    console.error(`${global_var.user_info.uname} 获取视频评论内容失败`, e);
                                }
                    ...
                    let pageurl = global_var.current_page.url();
                    if (pageurl.includes("www.bilibili.com") && lottery_setting.prevent_module.share_video_switch) {
                        ...
                        if (share_video_list.length > 0) {
                            for (let video_elem of share_video_list) {
                                try {
                                    ...
                                    await pptr_op.check_page_is_front(global_var.current_page);
                                    await global_var.current_page.goto(video_elem);
                                    try {
                ...
                create_word_dynamic_from_dynamic_main_page: async function (content_list, create_times) {
                    ...
                    if (!global_var.current_page.url().includes("t.bilibili.com")) {
                        await global_var.current_page.goto("https://t.bilibili.com/?spm_id_from=333.1007.0.0");
                    }
                    ...
                    for (let i = 0; i < create_times; i++) {
                        ...
                        if (typeof content != "string" || !content || content.includes("undefined") || content.includes("null") || content.includes("true") || content.includes("false")) {
                            //检查是否传入的是string类型参数 或者是否为空
                            continue;
                        }
                        if (!global_var.current_page.url().includes("t.bilibili.com")) await global_var.current_page.goto("https://t.bilibili.com/?spm_id_from=333.1007.0.0", {
                            waitUntil: "networkidle0",
                        });
                        for (let i = 0; i < 5; i++) {
                            let textarea = await global_var.current_page.$(".bili-rich-textarea");
                            await textarea.click();
                            await global_var.current_page.focus(".bili-rich-textarea");
                            await sleep(1e3);
                            await textarea.type(content, {
                                delay: 20,
                            });
                            await sleep(1e3);
                            let textarea_content = await global_var.current_page.$eval(".bili-rich-textarea", (el) => el.textContent);
                            textarea_content = textarea_content.trim();
                            if (utl.remove_invisible_char(textarea_content
                                .slice(1)
                                .replaceAll(/[\uD800-\uDBFF][\uDC00-\uDFFF]/g, "")) == utl.remove_invisible_char(content.replaceAll(/[\uD800-\uDBFF][\uDC00-\uDFFF]/g, ""))) {
                                await sleep(utl.random_choice(3 * lottery_setting.prevent_module.share_video_sleep_time));
                                await global_var.current_page.click(".bili-dyn-publishing__action.launcher");
                                console.log(`${global_var.user_info.uname}\t点击了发布动态`);
                                let check_btn;
                                try {
                                    check_btn = await global_var.current_page.$(".bili-dyn-specification-popup__btn.bili-button.primary.bili-button--medium");
                                    if (check_btn) {
                ...
                create_topic_dynamic_from_dynamic_main_page: async (create_times, discuss_content, copy_discuss_flag) => {
                    if (typeof create_times != "number") {
                        create_times = 1;
                    }
                    if (!(await global_var.current_page.url()).includes("t.bilibili.com")) {
                        await global_var.current_page.goto("https://t.bilibili.com/");
                    }
                    let relevant_topic__titles = await global_var.current_page.$$(`.relevant-topic__title`);
                    for (let i = 0; i < create_times; i++) {
                        let extract_topic_title;
                ...
                ignore_replies: [//无视掉的抄评论词
                    `转发了`, `转发动态`, `秋梨膏`, `我我我`, `永不缺席`, `永不中奖`, `永不放弃`, `好运`, `说不定呢`, `冲`, `凑热闹`, `永不缺席`, `无所谓`, "来了", "期待", "好", "来了来了", "好好好", "抽我", "抽我抽我", "下午好", "早上好", "中午好", "晚上好", "重在参与", "许愿", "加油点赞", "支持支持", "支持", "好耶", "1", "不错啊", "许愿呀", "锦鲤附体", "用自己的微薄之力给up撑腰", "冲冲冲", "做个梦", "幸运儿来啦！", "来力来力", "坚持不懈，迎难而上，开拓创新！", "我", "中", "来力", "开心", "可以", "来啦", "万一呢", "加油加油!", "加油加油！", "点赞", "真棒", "坚持不懈，迎难而上", "谢谢宠粉祝粉丝越来越多发展越来越好", "大家注意看，这是", "他真是太宠粉了，请多点点关注", "许个愿，我永远支持up主，祝愿你的粉丝越来越多，感谢有你啊", "希望你们中", "我是天选之子", "太酷了！！！！必须支持",],
                /**
                 *
                 ...
                 * @returns
                 */
                get_copy_reply: async (dynamic_id_or_BVid, mode, pn_percent, get_api_reply_resp_flag, dynamic_content = "") => {
                    //，获取的评论是去掉了@和表情包的
                    ...
                    for (let _ = 0; _ < loop_times; _++) {
                        //超过就退出,进行随机抽取
                        let resp = await my_operator.copy_reply_module.get_reply_list(dynamic_id_or_BVid, mode, pn_percent, get_api_reply_resp_flag, dynamic_content, pn_list);
                        all_replies_content = all_replies_content.concat(resp.ret_list);
                        ...
                        if (all_replies_content.length <= 15) {
                            ...
                            resp = await my_operator.copy_reply_module.get_reply_list(dynamic_id_or_BVid, mode, Math.random(), true, dynamic_content, pn_list);
                            all_replies_content = all_replies_content.concat(resp.ret_list);
                            pn_list.push(resp.pn);
                        }
                        if (all_replies_content.length <= 15) {
                        ...
                        console.log(`https://t.bilibili.com/${dynamic_id_or_BVid} ${global_var.user_info.uname}获取到的所有评论，获取了 ${pn_list} 页数\n总获取次数：${loop_times}次！`, new Date());
                 ...
                 */
                get_reply_list: async (dynamic_id_or_BVid, mode, pn_percent, get_api_reply_resp_flag, dynamic_content = "", pn_list = []) => {
                    ...
                    if (!String(dynamic_id_or_BVid).toUpperCase().includes("BV")) {
                        ...
                        if (dynDetail_data == undefined || dynDetail_data == -412 || global_var.response.reply_main == undefined) {
                            ...
                            try {
                                ...
                                console.error(e, "get_reply_list失败", global_var.user_info.uname);
                            }
                        } else {
                    ...
                    if (get_api_reply_resp_flag || global_var.response.reply_main == undefined || get_api_fail) {
                        ...
                        if (!reply_main_res.code) {
                            ...
                            console.error(`评论api获取数据失败！${JSON.stringify(reply_main_res)}`);
                        }
                    } else {
                    ...
                    for (let repindex = 0; repindex < replies.length; repindex++) {
                        ...
                        replies_content[repindex] = utl
                            .remove_emoji_topic_at(replies[repindex].content.message.replaceAll(replies[repindex].member.uname, //替换at的自己的用户名
                                global_var.user_info.uname == undefined ? "" : global_var.user_info.uname), dynamic_content)
                            .trim();
                    ...
                    for (let i of replies_content) {
                        if (i) {
                            promise_list.push(my_operator.copy_reply_module
                                .string_semantic(i)
                                .then((resp) => {
                                    if (resp) {
                                        newArr.push(i);
                ...
                wenxin_paraphrase: async (input_str) => {
                    ...
                    try {
                        ...
                        let res = await MYAPI.BiliAPI.post(`${global_var.Baidu_wenxin.paraphrase_api}?access_token=${global_var.Baidu_wenxin.access_token}`, {
                            text: input_str, async: 1, min_dec_len: 1, seq_len: 128, topp: 0.8, typeId: 1,
                        });
                        if (res.code != 0) {
                ...
                Paraphase_nlpcda: async (OriginMessage) => {
                    ...
                    while (1) {
                        try {
                            let res = await axios.post("http://localhost:5555/v1/sync/ai_reply", {
                                prompt: OriginMessage,
                                user: global_var.user_info.uname,
                                dynamic_url: "https://www.bilibili.com/opus/" + global_var.dynamic_id,
...
答：`;
                            ...
                            console.log({
                                ...
                                user: global_var.user_info.uname,
                                dynamic_url: await global_var.current_page.url(),
                                request_time: Math.ceil(Date.now() / 1000),
                ...
                copy_reply_judge: (dynamic_content) => {
                        ...
                        if (global_var.response.reply_main) {
                            ...
                            if (rep_content_list.length > 3) {
                                for (let origin_msg of rep_content_list) {
                                    let similarity = {
                                        similar_content: undefined, score: 0,
                                    };
                                    ...
                                    if (similarity.score < 0.8) {
                                        similar_list.push({
                                            similar_msg: origin_msg, counter: 1,
                                        });
                                    } else {
                                        similar_list.map((e) => {
                    ...
                    dynamic_content = this.my_operator.dynamic_comment_operator.pre_process_dynamic_content(dynamic_content);
                    let manual_re67 = /.*[评|带]((?!抽奖|,|，|来).){0,7}“|.*[评|带]((?!抽奖|,|，|来).){0,7}"|.*[评|带]((?!抽奖|,|，|来).){0,7}【|.*[评|带]((?!抽奖|,|，|来).){0,7}：|.*[评|带]((?!抽奖|,|，|来).){0,7}：|.*[评|带]((?!抽奖|,|，|来).){0,7}「|.*带关键词.{0,7}"|.*评论关键词[“”‘’"']|.*留言((?!抽奖|,|，|来).){0,7}“|.*对出.{0,10}下联.{0,5}横批|.*回答.{0,8}问题|.*留下.{0,10}祝福语|.*留下.{0,10}愿望|.*找到.{0,10}不同的.{0,10}留言|.*答案放在评论区|.*几.{0,5}呢？|.*有奖问答|.*想到.{0,19}关于.{0,20}告诉|.*麻烦大伙评论这个|报暗号【.{0,4}】|评论.{0,3}输入.{0,3}["“”:：]|.*评论.{0,7}暗号/gim.test(dynamic_content);
                    let manual_re76 = /.*留言((?!抽奖|,|，|来).).{0,7}"|.*留下((?!抽奖|,|，|来).){0,5}“|.*留下((?!抽奖|,|，|来).){0,5}【|.*留下((?!抽奖|,|，|来).){0,5}：|.*留下((?!抽奖|,|，|来).){0,5}「/gim.test(dynamic_content);
                    let manual_re77 = /.*留言((?!抽奖|,|，|来).).{0,7}"|.*留言((?!抽奖|,|，|来).).{0,7}“|.*留言((?!抽奖|,|，|来).){0,7}【|.*留言((?!抽奖|,|，|来).){0,7}：|.*留言((?!抽奖|,|，|来).){0,7}「/gim.test(dynamic_content);
                    let manual_re6 = /.*@TA|.*@.{0,15}朋友|.*艾特|.*@.{0,3}你的|.*标记.{0,10}朋友|.*@{0,15}赞助商|.*发表你的新年愿望\+个人的昵称|.*抽奖规则请仔细看图片|.*带上用户名|.*活动详情请戳图片|.*@个人用户名|评论.{0,5}附带.{0,10}相关内容|回复.{0,5}视频.{0,10}相关内容|.*评论.{0,5}昵称/gim.test(dynamic_content);
                ...
                para_phase_judge: (dynamic_content) => {
                    ...
                    dynamic_content = dynamic_content.replaceAll(/【关注】/gim, "");
                    dynamic_content = dynamic_content.replaceAll(/\?/gim, "？");
                    let manual_re67 = /.*[评|带]((?!抽奖|,|，|来).){0,7}“|.*[评|带]((?!抽奖|,|，|来).){0,7}"|.*[评|带]((?!抽奖|,|，|来).){0,7}【|.*[评|带]((?!抽奖|,|，|来).){0,7}:|.*[评|带]((?!抽奖|,|，|来).){0,7}：|.*[评|带]((?!抽奖|,|，|来).){0,7}「|.*带关键词.{0,7}"|.*评论关键词[“”‘’"']|.*留言((?!抽奖|,|，|来).){0,7}“|.*对出.{0,10}下联.{0,5}横批|.*回答.{0,8}问题|.*留下.{0,10}祝福语|.*留下.{0,10}愿望|.*找到.{0,10}不同的.{0,10}留言|.*答案放在评论区|.*几.{0,5}呢？|.*有奖问答|.*想到.{0,19}关于.{0,20}告诉|.*麻烦大伙评论这个|留下.{0,7}的/gim.test(dynamic_content);
                    let manual_re76 = /.*留下((?!抽奖|,|，).){0,5}“|.*留下((?!抽奖|,|，).){0,5}【|.*留下((?!抽奖|,|，).){0,5}:|.*留下((?!抽奖|,|，).){0,5}：|.*留下((?!抽奖|,|，).){0,5}「/gim.test(dynamic_content);
                    let manual_re77 = /.*留言((?!抽奖|,|，).).{0,7}“|.*留言((?!抽奖|,|，).){0,7}【|.*留言((?!抽奖|,|，).){0,7}:|.*留言((?!抽奖|,|，).){0,7}：|.*留言((?!抽奖|,|，).){0,7}「/gim.test(dynamic_content);
            ...
            browserSetting: {
                ...
                getCookie: async (cookiefilename) => {
                    ...
                    if (fs.existsSync(__dirpath + path)) {
                        data = fs
                            .readFileSync(__dirpath + path, function (err, data) {
                                if (err) {
                                    console.log(err);
                                    throw err;
                                }
                    ...
                    let path = `cookie_file/${cookiefilename}.txt`;
                    let cookie = await global_var.current_page.cookies("https://bilibili.com");
                    let ckStr = "";
                ...
                waitForResponse: async (page, url_include) => {
                    try {
                        return await page
                            .waitForResponse((response) => response.url().includes(url_include) && response.status() === 200)
                            .then(async (response) => await response.json())
                            .catch((e) => {
                                console.error(`${global_var.user_info.uname}\t等待响应${url_include}失败\t${new Date().toLocaleTimeString()}`);
                            });
                    } catch (e) {
                        console.error(`${global_var.user_info.uname}\t等待响应${url_include}失败\t${new Date().toLocaleTimeString()}`);
                        throw Error(`${global_var.user_info.uname}\t等待响应${url_include}失败\t${new Date().toLocaleTimeString()}`);
                    }
                },
            },
        };

        let lottery_setting = await utl.get_lottery_setting(this.account_name, this.uid);
        this.lottery_setting = this.lottery_setting ? this.lottery_setting : lottery_setting;
...
module.exports = {ENVIRONMENT, usage_map};
