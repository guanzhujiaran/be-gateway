    ...
    for (let retry_times = 0; retry_times < 3; retry_times++) {
        try {
            for (let operation of operations) {
                let { ele, optype, wait } = operation;
                // 根据操作类型执行相应的操作
                await sleep(parseInt(wait) / 2)
                if (optype === 'click') {
                    page.click(ele,
                        { 'delay': 300, })
                        .then(
                            () => {
                                console.log(`click the element:${ele} success`);
                            },
                            (reject) => {
                                new Error('点击失败', reject)
                            }
                        )
                } else if (optype === 'type') {
                    await page.type(ele, '[doge]');
                }
                else if (optype === 'tap') {
                    page.tap(ele,
                        { 'delay': 300, })
                        .then(
                            () => {
                                console.log(`tap the element:${ele} success`);
                            },
                            (reject) => {
                                new Error('点击失败', reject)
                            }
            ...
            console.warn(`${await page.url()}\t点击失败！\n${e}`);
            await sleep(3e3)
 ...
 * 执行给定的操作列表
 * @param {object} page - Puppeteer 页面对象
 * @param {array} operationsList - 操作列表数组
 */
async function executeOperations(page, operationsList) {
    try {

        for (const operations of operationsList) {
            const { url, ops, emulate_device } = operations;
            let new_page = await (await page.browser()).newPage();
            let emulate_device_detail = devices.filter(el => el.name == emulate_device).shift();
            if (emulate_device && emulate_device_detail) {
                await new_page.emulate(emulate_device_detail)
            }
            // 导航到指定的 URL
            for (let try_time = 0; try_time < 3; try_time++) {
                try {
                    await new_page.goto(url, { waitUntil: 'networkidle0' });
                    break;
                }
                catch (e) {
                    console.warn(`前往页面${url} 失败`, e);
                }
            }

            // 执行操作列表
            await performOperations(new_page, ops, url);
        }
    } catch (error) {
        console.error('参加点击的活动\tAn error occurred:', error);
    }
}
