...
class chatgptOP {
    ...
    __check_page_wrapper(func) {
        return async (...args) => {
            try {
                ...
                console.warn('chatgpt_error', e, this, arguments);
    ...
    async __typemsg(inputmsg) {
        ...
        for (let sepmsg of inputmsg.split('\n')) {
            await this.chatpage.type('#prompt-textarea', String(sepmsg), { delay: 20 })
    ...
    async __get_answer() {
        if (await this.chatpage.$(`.py-2.px-3.border.text-gray-600.rounded-md.text-sm`)) {
            throw ('获取回复出错')
        }
        let ret_answer = await this.chatpage.$$eval('.markdown.prose.w-full', els => els.at(-1).textContent);
    ...
    async __waitfor_resp(includes_url) {
        return await this.chatpage.waitForResponse(response => response.url().includes(includes_url), { timeout: 30e3 })
    }
