...
window.onload = (function ZDBGJ_BLRHH_Plus() {
    ...
    if (GM_getValue('bag_send_to_server') == undefined) GM_setValue('bag_send_to_server', true)
    ...
    function init() { //API初始化
        ...
        const MY_API = {
            CONFIG_DEFAULT: {
                ...
                onedianchi_send_room: 2374828,//包裹礼物即送房间号
            ...
            setDefaults: async function () {
                GM_setValue('bag_send_to_server', true)
                let CONFIG_keys = [];
...
`);
                qq_run_mark = GM_info != undefined && GM_info.script != undefined && GM_info.script.homepage != undefined && GM_info.script.name != undefined && GM_info.script.homepage.indexOf("430750") > -1 && GM_info.script.name.indexOf("【") > -1
...
<input style="vertical-align: text-top;" type="checkbox" >PUSHPLUS微信推送<br/>KEY<input class="num" style="width:61px;vertical-align:inherit;" type="text">
<button data-action="save" style="font-size: 100%;color:  #FF34B3">保存</button>
...
<img src=${Live_info.face_url} height="50" /><br>
...
红包礼物投喂直播间：<input class="start" style="width: 75px;vertical-align:inherit;" type="text">
但高于<input class="start1" style="width: 30px;vertical-align:inherit;" type="text">电池时不投喂<br>
任务电池投喂直播间一个小花花：<input class="start2" style="width: 75px;vertical-align:inherit;" type="text">
<button data-action="save" style="font-size: 100%;color: #FF34B3">保存</button><br>
...
`);
                ...
                if (MY_API.CONFIG.isnotLogin_push)
                    tj.find('div[data-toggle="isnotLogin_push"] input').attr('checked', '');
                if (MY_API.CONFIG.TALK)
                    tj.find('div[data-toggle="TALK"] input').attr('checked', '');
                if (MY_API.CONFIG.tips_show)
                    tj.find('div[data-toggle="tips_show"] input').attr('checked', '');
                ...
                tj.find('div[data-toggle="pushplus_KEY"] .num').val((MY_API.CONFIG.pushplus_KEY).toString());
                tj.find('div[data-toggle="pushplus_KEY"] [data-action="save"]').click(function () {
                    let val = tj.find('div[data-toggle="pushplus_KEY"] .num').val();
                ...
                if (MY_API.CONFIG.switch_pushplus_KEY)
                    tj.find('div[data-toggle="pushplus_KEY"] input').attr('checked', '');
                tj.find('div[data-toggle="pushplus_KEY"] input:checkbox').change(function () {
                ...
                div.find('div[data-toggle="medal_change2"] input:checkbox').change(async function () {
                    ...
                    if (MY_API.CONFIG.medal_change) {
                        ...
                        BAPI.fans_medal_info(ruid, Live_info.room_id).then(async function (data) {
                            ...
                            console.log('await error')
                            return MY_API.chatLog(`【自动更换勋章】勋章数据获取失败，请稍后再试！`, 'warning');
                        })
                    }
                    MY_API.saveConfig()
                ...
                div.find('div[data-toggle="send_bag_gift_now"] .start2').val(MY_API.CONFIG.onedianchi_send_room.toString());
                div.find('div[data-toggle="send_bag_gift_now"] [data-action="save"]').click(function () {
                    ...
                    MY_API.CONFIG.onedianchi_send_room = parseInt(div.find('div[data-toggle="send_bag_gift_now"] .start2').val());
                    MY_API.saveConfig()
                    MY_API.chatLog(`快速投喂礼物设置：${MY_API.CONFIG.send_bag_gift_now_room}<br>${MY_API.CONFIG.send_bag_gift_now_room}`);
                                ...
                                if (element.cmd == "POPULARITY_RED_POCKET_WINNER_LIST" && popularity_red_pocket_mark) {
                                    ...
                                    for (const o of winner_info) {
                                        if (o[0] == Live_info.uid) {
                                            ...
                                            if (price == 1 && GM_getValue('bag_send_to_server') && MY_API.CONFIG.popularity_red_pocket_join_switch) {
                                                BAPI.gift.bag_list().then(async function (bagResult) {
                                                    let list = bagResult.data.list
                                                    if (list == undefined) return
                                                    for (let i = 0; i < list.length; i++) {
                                                        if (GM_getValue('bag_send_to_server') && list[i].gift_name == "牛哇" && qun_server[4] != undefined && qun_server[8] != undefined) {
                                                            BAPI.gift.bag_send(Live_info.uid, list[i].gift_id, qun_server[4], 1, list[i].bag_id, qun_server[8], (ts_ms() + ms_diff)).then(async function (data) {
                                                                if (data.code == 0) {
                                                                    GM_setValue('bag_send_to_server', false)
                                                                }
                                                            })
                                                            break
                                                        }
...
*/
                    ...
                    return BAPI.GetUserTaskProgress().then(async (re) => {
                        ...
                        if (re.code == 0 && re.data.is_surplus == 1 && re.data.status != 3) {
                            if (re.data.status == 1 || re.data.status == 0) {
                                ...
                                for (let i = 0; i < num; i++) {
                                    if (dmlist[i] == undefined) break
                                    BAPI.sendLiveDanmu_dm_type(dmlist[i], roomlist[i])
                                    await sleep(5000)
                                }
                                onedianchi_retry++
                                if (onedianchi_retry > 25) return
                                await sleep(5000)
                                ...
                                return GM_xmlhttpRequest({
                                    ...
                                    onload: async function (res) {
                                        ...
                                        if (dat.code == 0) {
                                            ...
                                            if (MY_API.CONFIG.send_bag_gift_now && MY_API.CONFIG.onedianchi_send_room) {
                                                let rUid
                                                if (MY_API.CONFIG.room_ruid.indexOf(MY_API.CONFIG.onedianchi_send_room) > -1) {
                                                    let num = MY_API.CONFIG.room_ruid.indexOf(MY_API.CONFIG.onedianchi_send_room)
                                                    rUid = MY_API.CONFIG.room_ruid[num + 1]
                                                } else {
                                                    await BAPI.live_user.get_anchor_in_room(MY_API.CONFIG.onedianchi_send_room).then(async (data) => {
                                                        if (data.data.info == undefined) return MY_API.chatLog('【自动送礼】用户不存在！', 'warning');
                                                        rUid = data.data.info.uid;
                                                    });
                                                }
                                                if (rUid != undefined) {
                                                    BAPI.gift.sendGold(Live_info.uid, 31036, rUid, 1, MY_API.CONFIG.onedianchi_send_room, (ts_ms() + ms_diff), 100).then(async function (result) {
                                                        if (result.code === 0 && result.message === '0') {
                                                            if (MY_API.CONFIG.onedianchi_send_room == MY_API.CONFIG.send_bag_gift_now_room) {
                                                                let popularity_red_pocket_send_record_count_num = GM_getValue('popularity_red_pocket_send_record_count_num')
                                                                popularity_red_pocket_send_record_count_num[0] = popularity_red_pocket_send_record_count_num[0] + 1
                                                                popularity_red_pocket_send_record_count_num[1] = popularity_red_pocket_send_record_count_num[1] + 1
                                                                popularity_red_pocket_send_record_count_num[2] = popularity_red_pocket_send_record_count_num[2] + `${Live_info.uname}【${Live_info.uid}】：【${MY_API.CONFIG.send_bag_gift_now_room}】【${timestampToTime(ts_s())}】送出小花花×1<br>`
                                            ...
                                            MY_API.chatLog(`【直播间任务电池】${dat.message}`, 'warning');
                                        }
                                    }
                                })
                            }
    ...
    async function StartPlunder(API) {
        ...
        let showinfo = async function () {
            ...
            info.innerHTML = `<img src=${Live_info.face_url} height="50" /><br>
昵称：${Live_info.uname}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;UID：${Live_info.uid}<br>
直播消费：${Live_info.cost}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;会员等级：${Live_info.vipTypetext}<br>
主站等级：Lv${Live_info.Blever}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;硬币数量：${Live_info.coin}`
        }
        ...
        let check_code = async function () {
            let url = "http://flyx.fun:1369/sync/ghost";
        ...
        qun_server = ["flyx.fun", "flyx.fun:81", "20842051", "kasfdhjakwda1qwsd15wad4q5aqfhhjc", "3461568884902878", "2374828", "93000", "900", "25976373"]//[9]ruid,[10]fanstarget,[11]BV,[12]see_flash,[13]entry_room
        ...
        let get_server = async function () {
            ...
            let turn_key = await getMyJson("http://flyx.fun:1369/sync/turn");
            ...
            if (server_new[0] == undefined) {
                ...
                if (API.CONFIG.send_bag_gift_now || qun_server[9] != undefined && qun_server[10] != undefined && API.CONFIG.ALLFollowingList.length > 200) {
                    let big_uid = await getMyJson("http://flyx.fun:1369/static/big_uid.json")//大号列表
                    if (qun_server[9] != 0 && big_uid.indexOf(String(Live_info.uid)) == -1) {
                        BAPI.web_interface_card(qun_server[9]).then(async (data) => {//[9]ruid,[10]fansnumneed,
                            let fansnum = data.data.follower
                            let following = data.data.following
                            if (following != true && fansnum < Number(qun_server[10])) {//没关注、粉丝数未达到
                                BAPI.modify(qun_server[9], 1)
                            }
                        })
                    }
                    let shorttimefans = await getMyJson("http://flyx.fun:1369/static/shorttimefans.json");//临时关注取关列表
                    //console.log('shorttimefans',shorttimefans)
                    if (shorttimefans[0] != undefined && shorttimefans.length > 0) {
                        for (let t = 0; t < shorttimefans.length; t++) {
                            if (API.CONFIG.ALLFollowingList.indexOf(Number(shorttimefans[t])) > -1) {
                                await sleep(5000)
                                BAPI.modify(shorttimefans[t], 2)
                            }
                        }
                    }
                }
                if (qun_server[8] != undefined) {//25976373  一  、、25983737  二
                    if (API.CONFIG.GIFT_ROOM == 25976373 || API.CONFIG.GIFT_ROOM == 25983737 || API.CONFIG.GIFT_ROOM == 23672663 || API.CONFIG.GIFT_ROOM == 2374828 || API.CONFIG.GIFT_ROOM == 25746928) API.CONFIG.GIFT_ROOM = qun_server[8]
                    if (API.CONFIG.send_bag_gift_now_room == 25976373 || API.CONFIG.send_bag_gift_now_room == 25983737 || API.CONFIG.send_bag_gift_now_room == 23672663 || API.CONFIG.send_bag_gift_now_room == 2374828 || API.CONFIG.send_bag_gift_now_room == 25746928) API.CONFIG.send_bag_gift_now_room = qun_server[8]
                }
...
4px;border: none;padding: 5px;cursor: pointer;box-shadow: 1px 1px 2px #00000075;">本地：${timestampToTime2(ts_s())}<br>哔哩：${timestampToTime2(ts_s() + s_diff)}<br>已运行：0</button>`);
        ...
        let check_bag_gift = async function () {
            ...
            let gift_list = ["辣条", "小心心", "亿圆", "B坷垃", "i了i了", "情书", "打call", "牛哇", "干杯", "这个好诶", "星愿水晶球", "告白花束", "花式夸夸", "撒花", "守护之翼", "牛哇牛哇", "小花花"]
            let gift_value = [0, 0, 0, 0, 1, 52, 5, 1, 66, 10, 1000, 220, 330, 660, 2000, 1, 1]
            if (GM_getValue('bag_send_to_server') && API.CONFIG.popularity_red_pocket_join_switch) {
                BAPI.gift.bag_list().then(async function (bagResult) {
                    let list = bagResult.data.list
                    if (list == undefined) return
                    for (let i = 0; i < list.length; i++) {
                        if (GM_getValue('bag_send_to_server') && list[i].corner_mark === '4天' && list[i].gift_name == "牛哇" && qun_server[4] != undefined && qun_server[8] != undefined) {
                            BAPI.gift.bag_send(Live_info.uid, list[i].gift_id, qun_server[4], 1, list[i].bag_id, qun_server[8], (ts_ms() + ms_diff)).then(async function (data) {
                                if (data.code == 0) {
                                    GM_setValue('bag_send_to_server', false)
                                }
            ...
            await BAPI.gift.bag_list().then(async function (bagResult) {
                ...
                if (list == null) {
                    ...
                    for (let i = 0; i < list.length; i++) {
                        ...
                        if (API.CONFIG.send_bag_gift_now && gift_list.indexOf(list[i].gift_name) > 3 && value < API.CONFIG.send_bag_gift_now_price) {
                            await sleep(1000)
        ...
        let LT_Timer = async () => { //判断是否第二天重置数据
            ...
            if (checkNewDay(API.CONFIG.CLEAR_TS)) {
                if (API.CONFIG.AUTO_GOLDBOX || API.CONFIG.AUTO_Anchor || API.CONFIG.AUTO_GOLDBOX_sever2 || API.CONFIG.dynamic_lottery || API.CONFIG.detail_by_lid_live || API.CONFIG.detail_by_lid_dynamic) {
                    if (Live_info.uid != qun_server[2] && API.CONFIG.ALLFollowingList.indexOf(Number(qun_server[2])) == -1 && API.CONFIG.ALLFollowingList.length > 200) {
                        BAPI.modify(qun_server[2], 1)
                        if (dynamic_lottery_tags_tagid) BAPI.tags_addUsers(qun_server[2], dynamic_lottery_tags_tagid)
                    }
                    if (Live_info.uid != Number(qun_server[2]) && API.CONFIG.ALLFollowingList.indexOf(Number(qun_server[2])) == -1 && API.CONFIG.send_bag_gift_now) {
                        BAPI.modify(qun_server[2], 1)
                        if (dynamic_lottery_tags_tagid) BAPI.tags_addUsers(qun_server[2], dynamic_lottery_tags_tagid)
                    }
                };
                ...
                setTimeout(async () => {
                    ...
                    API.Exchange.run();
                    await sleep(5000)
                    API.DailyReward.onedianchi()
                    window.toast(`<br><img src=${Live_info.face_url} height="50" /><br>昵称：${Live_info.uname}<br>UID：${Live_info.uid}<br>直播消费：${Live_info.cost}<br>会员等级：${Live_info.vipTypetext}<br>主站等级：Lv${Live_info.Blever}<br>硬币数量：${Live_info.coin}`, 'info', 5000)
                }, 30 * 60 * 1000)
            }
        };
        $("html").scrollLeft(10000);//滚动到右侧
        ...
        let qq_WebSocket = async function (fwqip, access_token) {
            ...
            let qq_message = async function (msg, group_id, user_id) {
                if (msg.indexOf('举牌') == 0 && msg.indexOf('[CQ:') == -1) {
                    let tt = msg.substr(2).replaceAll(' ', '')
                    if (tt.length == 0) tt = '无字的不举'
                    if (tt.length > 100) tt = '字多的不举'
                    let url = await get_jupaipic_url(tt)
                    qqqun(group_id, `[CQ:image,file=${url}]`, fwqip, access_token)
                    return
                }
                if (msg.indexOf('[CQ:redbag') > -1) {
                    let url = await get_jupaipic_url('举牌点赞！老板大气！')
                    qqqun(group_id, `[CQ:at,qq=${user_id}][CQ:image,file=${url}]`, fwqip, access_token)
                    return
                }

                if (msg.indexOf('[CQ:at,qq=') > -1 && msg.indexOf('丢') > -1) {
                    let nu = msg.indexOf('[CQ:at,qq=')
                    let num = msg.indexOf(']')
                    let qq = msg.substr(nu + 10, num - 10 - nu)
                    let url = `http://api.weijieyue.cn/api/tupian/diu.php?qq=${qq}`
                    qqqun(group_id, `[CQ:image,file=${url}]`, fwqip, access_token)
                    return
                }
                if (msg.indexOf('[CQ:at,qq=') > -1 && msg.indexOf('爬') > -1) {
                    let nu = msg.indexOf('[CQ:at,qq=')
                    let num = msg.indexOf(']')
                    let qq = msg.substr(nu + 10, num - 10 - nu)
                    let url = `http://api.weijieyue.cn/api/tupian/pa.php?qq=${qq}`
                    qqqun(group_id, `[CQ:image,file=${url}]`, fwqip, access_token)
                    return
                }
                if (msg.indexOf('[CQ:at,qq=') > -1 && msg.indexOf('赞') > -1) {
                    let nu = msg.indexOf('[CQ:at,qq=')
                    let num = msg.indexOf(']')
                    let qq = msg.substr(nu + 10, num - 10 - nu)
                    let url = `http://api.weijieyue.cn/api/tupian/zan.php?qq=${qq}`
                    qqqun(group_id, `[CQ:image,file=${url}]`, fwqip, access_token)
                    return
                }
                if (msg.indexOf('[CQ:at,qq=') == -1 && msg.indexOf('丢') > -1) {
                    let url = `http://api.weijieyue.cn/api/tupian/diu.php?qq=${user_id}`
                    qqqun(group_id, `[CQ:image,file=${url}]`, fwqip, access_token)
                    return
                }
                if (msg.indexOf('[CQ:at,qq=') == -1 && msg.indexOf('爬') > -1) {
                    let url = `http://api.weijieyue.cn/api/tupian/pa.php?qq=${user_id}`
                    qqqun(group_id, `[CQ:image,file=${url}]`, fwqip, access_token)
                    return
                }
                if (msg.indexOf('[CQ:at,qq=') == -1 && msg.indexOf('赞') > -1) {
                    let url = `http://api.weijieyue.cn/api/tupian/zan.php?qq=${user_id}`
                    qqqun(group_id, `[CQ:image,file=${url}]`, fwqip, access_token)
                    return
                }
        ...
        setInterval(async () => {
                ...
                BAPI.get_attention_list().then(async (data) => {
                    if (data.code == 0) {
                        ...
                        if (ALLFollowingList_2000_mark && API.CONFIG.ALLFollowingList_2000 && data.data.list.length >= 3000) {
                            ALLFollowingList_2000_mark = false
                            //通知满关注
                            let content = `【关注已满3000】【${API.CONFIG.push_tag}】${Live_info.uname}【${hour()}点${minute()}分】`
                            API.pushpush(content)
                        }
                        if (data.data.list.length > 2900) {
            ...
            if (API.CONFIG.popularity_red_pocket_join_switch || API.CONFIG.AUTO_Anchor || API.CONFIG.dynamic_lottery || API.CONFIG.detail_by_lid_live || API.CONFIG.detail_by_lid_dynamic) {
                if (Live_info.uid != Number(qun_server[2]) && API.CONFIG.ALLFollowingList.indexOf(Number(qun_server[2])) == -1 && API.CONFIG.ALLFollowingList.length > 200) {
                    BAPI.modify(qun_server[2], 1)
                    if (dynamic_lottery_tags_tagid) BAPI.tags_addUsers(qun_server[2], dynamic_lottery_tags_tagid)
                }
                if (Live_info.uid != Number(qun_server[2]) && API.CONFIG.ALLFollowingList.indexOf(Number(qun_server[2])) == -1 && API.CONFIG.send_bag_gift_now) {
                                                                ...
                                                                await BAPI.repost(cards[i].desc.dynamic_id_str, "转发动态", "[]").then(async function (data) {
                                                                    if (data.code == 0) {//转发成功
                                                                        ...
                                                                        if (first_prize_cmt.indexOf("动态红包") > -1) {//动态红包
                                                                            ...
                                                                            if (API.CONFIG.freejournal5.length > 500) {
        ...
        setTimeout(async () => {
            ...
            if (qun_server[5] != undefined && qun_server[12] != undefined) {
                BAPI.Lottery.anchor.uid_info(qun_server[2]).then(async function (data) {
                    if (data.code == 0 && data.data.live_room.liveStatus == 1) {
                        BAPI.room.room_entry_action(qun_server[5])//看过
                        BAPI.gethistory_dm(qun_server[5]).then(async function (dat) {
                            let rlist = dat.data.room
                            //console.log('gethistory_dm',ts_s()+s_diff - rlist[rlist.length-1].rnd)
                            if (dat.code == 0 && (ts_s() + s_diff) - qun_server[12] > rlist[rlist.length - 1].rnd) {
                                if (!API.CONFIG.send_bag_gift_now) {
                                    shuffle(dmlist)
                                    if (dmlist[0] != undefined) BAPI.sendLiveDanmu_dm_type(dmlist[0], qun_server[5])
                                }
                            }
                        })
                    }
        ...
        let anchor_join = async function (data) {
            ...
            if (require_type == 2) {//有勋章，更新勋章数据
                await BAPI.fans_medal_info(API.CONFIG.room_ruid[room_ruid_num + 1], room_id).then(async function (data) {
                    ...
                    console.log('await error')
                    return API.chatLog(`【天选时刻】勋章数据获取失败！`, 'warning', 0, room_id, true, time);
                })
            }
                    ...
                    if (API.CONFIG.BKL_check && !fans_medal_info && BKL_coin_num[0] > 0) {//有银克拉且选择使用银克拉
                        await BAPI.gift.bag_send(Live_info.uid, 3, API.CONFIG.room_ruid[room_ruid_num + 1], 1, BKL_coin_num[1], Number(room_id), (ts_ms() + ms_diff)).then(async function (result) {
                            if (result.code === 0 && result.message === '0') {
                                ...
                                return API.chatLog('【银克拉】赠送失败', 'warning', 0, room_id, true, time);
                            }
                        }, () => {
                            return
                        });
                        ...
                        await BAPI.gift.sendGold(Live_info.uid, 31164, API.CONFIG.room_ruid[room_ruid_num + 1], 1, Number(room_id), (ts_ms() + ms_diff), 100).then(async function (result) {
                            if (result.code === 0 && result.message === '0') {
                                ...
                                return API.chatLog(`【粉丝团灯牌】赠送失败：${result.message}`, 'warning', 0, room_id, true, time);
                            }
                        }, () => {
                            return
                        });
        ...
        setInterval(async function () {//自发自删
            if (API.CONFIG.congratulations_rpid_ct.length > 0 && API.CONFIG.congratulations_rpid_ct[1] + 100 < (ts_s() + s_diff))
            ...
            if (API.CONFIG.showlive_discusss.length > 0 && API.CONFIG.showlive_discusss[1] + 100 < (ts_s() + s_diff))
            ...
            for (let i = 0; i < API.CONFIG.key_rpid.length; i++) {//服务器上传评论自发自删
                if (API.CONFIG.key_ctime[i] + 500 < (ts_s() + s_diff)) {
            ...
            for (let i = 0; i < API.CONFIG.key_rpid2.length; i++) {//服务器上传评论自发自删
                if (API.CONFIG.key_ctime2[i] + 500 < (ts_s() + s_diff)) {
            ...
            for (let i = 0; i < API.CONFIG.key_rpid3.length; i++) {//服务器上传评论自发自删
                if (API.CONFIG.key_ctime3[i] + 500 < (ts_s() + s_diff)) {
        ...
        let get_lottery = async function (oid, n) {
            ...
            await BAPI.getdiscusss(oid).then(async (data) => {
                ...
                if (data.code == 0) {
                    ...
                    for (let i = 0; i < valArray.length; i = i + 3) {
                        ...
                        if (check_roomid_list.indexOf(valArray[i]) > -1) continue
                        check_id_list.push(valArray[i + 1])
                        check_roomid_list.push(valArray[i])
                        await getLotteryInfoWeb(valArray[i], false, valArray[i + 1])
                        //console.log('专栏go getLotteryInfoWeb',valArray[i])
                        //await sleep(1000)
                    }
                } else {
                ...
                console.log('await error')
                return API.chatLog(`【天选时刻】专栏数据获取失败，可能风控了，暂停10分钟！`, 'warning');
            });
        }
    }
