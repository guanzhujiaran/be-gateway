...
let my_operator = {
    ...
    dynamic_comment_operator: {//回复内容相关操作
        pre_msg_processing: function (dynamic_content) {
            ...
            if (/.*带话题#.*#((?!投稿).)*$/gmi.test(dynamic_content) || /.*带.{0,5}#/.test(dynamic_content) || topobj2 || topobj3 || topobj4 || topobj5 || topobj10 || topobj12 || topobj13 || topobj14) {
                if (!premsg.includes('#')) {
                    utl.my_throw('话题获取失败')
                    return undefined
                }
        ...
        manual_reply_judge: function (dynamic_content) {
            ...
            let manual_re75 = /.*本周话题|.*互动话题|.*互动留言|.*互动时间|.*征集.{0,10}名字|.*投票.{0,5}选.{0,10}最.{0,5}的|.*一人说一个谐音梗/gmi.test(dynamic_content)
        ...
        key_word_reply: function (dynamic_content) {
            if (
                ...
                || /.*@TA|.*@.{0,15}朋友|.*艾特|.*@.{0,3}你的|.*标记.{0,10}朋友|.*@{0,15}赞助商|.*发表你的新年愿望\+个人的昵称|.*抽奖规则请仔细看图片|.*带上用户名|.*此视频|.*视频评论区|.*活动详情请戳图片/gmi.test(dynamic_content)
    ...
    prevent_filter_module: {
        ...
        create_word_dynamic_from_dynamic_main_page: async function (content_list, create_times) {
            ...
            if (!!(now.getHours() <= 23 && now.getHours() >= 5)) {
                console.log(`${global_var.user_info.uname}\t5点到23点不分享文字动态\t${(new Date()).toLocaleTimeString()}`)
                return
            }
    ...
    copy_reply_module: {//抄评论模块
        ...
        copy_reply_judge: (dynamic_content) => {
            ...
            let manual_re77 = /.*留言((?!抽奖|,|，).).{0,7}“|.*留言((?!抽奖|,|，).){0,7}【|.*留言((?!抽奖|,|，).){0,7}:|.*留言((?!抽奖|,|，).){0,7}：|.*留言((?!抽奖|,|，).){0,7}「/gmi.test(dynamic_content)
            let manual_re6 = /.*@TA|.*@.{0,15}朋友|.*艾特|.*@.{0,3}你的|.*标记.{0,10}朋友|.*@{0,15}赞助商|.*发表你的新年愿望\+个人的昵称|.*抽奖规则请仔细看图片|.*带上用户名|.*此视频|.*视频评论区|.*活动详情请戳图片/gmi.test(dynamic_content)
            let manual_re75 = /.*本周话题|.*互动话题|.*互动留言|.*互动时间|.*征集.{0,10}名字/gmi.test(dynamic_content)
...
let lottery_settingstr = await lottery_setting_file_reader('lottery_setting14')
let lottery_setting;
